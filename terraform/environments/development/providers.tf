...
#
# The endpoint is localhost, not `fakecloud`, because OpenTofu runs on the
# developer's host against the loopback publication in compose.yaml. Nothing in
# this directory is reachable from inside the `core` container.
