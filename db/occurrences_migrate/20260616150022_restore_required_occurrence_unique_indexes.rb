...
class RestoreRequiredOccurrenceUniqueIndexes < ActiveRecord::Migration[8.2]
  disable_ddl_transaction!

  def up
    add_index(:email_occurrences, :body, unique: true, name: "index_email_occurrences_on_body", algorithm: :concurrently) unless
      index_exists?(:email_occurrences, name: "index_email_occurrences_on_body")
    add_index(:ip_occurrences, :body, unique: true, name: "index_ip_occurrences_on_body", algorithm: :concurrently) unless
      index_exists?(:ip_occurrences, name: "index_ip_occurrences_on_body")
    add_index(:jwt_occurrences, :body, unique: true, name: "index_jwt_occurrences_on_body", algorithm: :concurrently) unless
      index_exists?(:jwt_occurrences, name: "index_jwt_occurrences_on_body")
    add_index(:telephone_occurrences, :body, unique: true, name: "index_telephone_occurrences_on_body", algorithm: :concurrently) unless
