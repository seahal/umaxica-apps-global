...
# adr/unified-enforcement.md: Enforcement Case, the policy SSOT for one org-realm
...
class CreateOrgEnforcementCases < ActiveRecord::Migration[8.2]
  def up
    create_table(:org_enforcement_cases) do |t|
      ...
      t.index(:kind)
      t.index(%i(state expires_at), name: "idx_org_enforcement_cases_state_expires_at")

      t.check_constraint(
        "kind IN ('security_lock', 'cooldown', 'temporary_freeze', 'permanent_ban', 'method_protection')",
        name: "chk_org_enforcement_cases_kind",
      )
      t.check_constraint(
        "state IN ('draft', 'pending_approval', 'active', 'ended', 'failed')",
        name: "chk_org_enforcement_cases_state",
      )
      t.check_constraint(
        "duration_mode IN ('timed', 'indefinite', 'permanent')",
        name: "chk_org_enforcement_cases_duration_mode",
      )
      t.check_constraint("visibility IN ('visible', 'hidden')", name: "chk_org_enforcement_cases_visibility")
      t.check_constraint(
        "release_mode IN ('automatic', 'operator', 'verification_required', 'break_glass_only')",
        name: "chk_org_enforcement_cases_release_mode",
      )
      t.check_constraint(
        "end_reason IS NULL OR end_reason IN (" \
        "'expired', 'revoked', 'superseded', 'corrected', 'appeal_approved', 'break_glass_released')",
        name: "chk_org_enforcement_cases_end_reason",
      )
      ...
      t.check_constraint(
        ...
        name: "chk_org_enforcement_cases_cooldown_duration",
      )
      t.check_constraint(
        "kind != 'permanent_ban' OR (duration_mode = 'permanent' AND expires_at IS NULL)",
        name: "chk_org_enforcement_cases_permanent_ban_duration",
      )
      t.check_constraint(
        "kind != 'temporary_freeze' OR duration_mode IN ('timed', 'indefinite')",
        name: "chk_org_enforcement_cases_temp_freeze_duration_mode",
      )
      t.check_constraint(
        "kind != 'temporary_freeze' OR duration_mode != 'indefinite' OR " \
        "(review_due_at IS NOT NULL AND release_mode = 'operator')",
        name: "chk_org_enforcement_cases_indefinite_freeze_review",
      )
      t.check_constraint("visibility != 'hidden' OR kind = 'permanent_ban'", name: "chk_org_enforcement_cases_hidden")
      t.check_constraint(
        "kind != 'security_lock' OR release_mode = 'verification_required'",
        name: "chk_org_enforcement_cases_security_lock_release",
      )
      ...
      t.check_constraint(
        ...
        name: "chk_org_enforcement_cases_approval_separation",
      )
      t.check_constraint(
        "principal_public_id != applied_by_operator_public_id",
        name: "chk_org_enforcement_cases_no_self_action",
      )
      t.check_constraint(
        "break_glass = false OR break_glass_approved_by_operator_public_id IS NOT NULL",
        name: "chk_org_enforcement_cases_break_glass_approver",
      )
    end
  end

  def down
    drop_table(:org_enforcement_cases)
  end
end
