class ConsolidateRetentionOnCustomerCredentials < ActiveRecord::Migration[8.0]
  def change
    safety_assured do
      ["customer_tokens", "customer_verifications", "customer_authorization_codes", "customer_reauth_sessions", "customer_secrets"].each do |t|
        if table_exists?(t)
