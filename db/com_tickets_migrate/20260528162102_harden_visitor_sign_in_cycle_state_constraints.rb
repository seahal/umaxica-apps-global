...
class HardenVisitorSignInCycleStateConstraints < ActiveRecord::Migration[8.2]
  STATE_CONSTRAINT = "chk_visitor_sign_in_cycles_status_state"
  STEP_CONSTRAINT = "chk_visitor_sign_in_cycles_status_step"

  def up
    add_constraint_unless_exists(:visitor_sign_in_cycles, status_state_sql, STATE_CONSTRAINT)
    add_constraint_unless_exists(:visitor_sign_in_cycles, status_step_sql, STEP_CONSTRAINT)
  end

  def down
    remove_check_constraint(:visitor_sign_in_cycles, name: STATE_CONSTRAINT, if_exists: true)
    remove_check_constraint(:visitor_sign_in_cycles, name: STEP_CONSTRAINT, if_exists: true)
  end
