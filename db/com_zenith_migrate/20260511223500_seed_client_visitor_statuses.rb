...
class SeedClientVisitorStatuses < ActiveRecord::Migration[8.2]
  IDS = [0, 1, 2, 3].freeze
  ...
  def up
    safety_assured do
      IDS.each do |id|
        execute(<<~SQL.squish)
          INSERT INTO client_visitor_statuses (id)
  ...
  def ensure_sequence!(max_id)
    sequence_name = select_value("SELECT pg_get_serial_sequence('client_visitor_statuses', 'id')")
