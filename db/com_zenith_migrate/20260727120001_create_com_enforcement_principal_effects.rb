...
class CreateComEnforcementPrincipalEffects < ActiveRecord::Migration[8.2]
  def up
    create_table(:com_enforcement_principal_effects) do |t|
      t.references(:com_enforcement_case, null: false, foreign_key: true, index: { unique: true })
      ...
      t.index(%i(principal_public_id ended_at), name: "idx_com_enforcement_principal_effects_principal_open")
    end
  end

  def down
    drop_table(:com_enforcement_principal_effects)
  end
end
