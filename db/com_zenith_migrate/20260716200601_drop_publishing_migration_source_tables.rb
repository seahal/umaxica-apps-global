...
# (20260613000001, 20260711010001) are kept in history and left unedited.
  ...
  def up
    safety_assured do
      ...
      CMS_FAMILIES.each do |family|
        CMS_TABLE_SUFFIXES.each { |suffix| drop_table("com_#{family}_#{suffix}", if_exists: true, force: :cascade) }
      end
    end
  end
