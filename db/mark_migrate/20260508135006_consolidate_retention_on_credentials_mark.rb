class ConsolidateRetentionOnCredentialsMark < ActiveRecord::Migration[8.0]
  def change
    safety_assured do
    tables = %w[user_verifications user_authorization_codes user_reauth_sessions user_secrets]
    tables.each do |t|
      if table_exists?(t)
