...
class AddExtendedPreferenceOptionsToPrincipal < ActiveRecord::Migration[8.2]
  TYPES = {
    currency: "currencies",
  ...
  def change
    add_local_columns(:user_preferences)

    create_preference_tables(:app, :app_preferences)
    create_preference_tables(:user, :user_preferences)
  end
