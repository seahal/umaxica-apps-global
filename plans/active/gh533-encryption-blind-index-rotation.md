...
**Active** (promoted from backlog, 2026-05-09). Replaces the earlier umbrella draft so that key
rotation is no longer indefinitely deferred.
...
| `staff_email`        | operator  | ❌ **missing**                                           |
| `staff_telephone`    | operator  | ❌ **missing**                                           |
| `customer_email`     | guest     | ✅ present (out of this plan's scope; see customer plan) |
| `customer_telephone` | guest     | ✅ present (same)                                        |

### `deterministic: true` declarations in scope

| File                               | Line | Note                              |
| ---------------------------------- | ---- | --------------------------------- |
| `app/models/concerns/email.rb`     | 22   | shared by user / staff / customer |
| `app/models/concerns/telephone.rb` | 20   | shared by user / staff / customer |
| `app/models/user_email.rb`         | 85   | per-model override                |
| `app/models/staff_email.rb`        | 72   | per-model override                |

The shared `Email` / `Telephone` concerns also drive the customer side. Step 4 below must be
sequenced with `plans/backlog/customer-email-telephone-encryption-plan.md` because flipping the
concern affects customer simultaneously.

## Sequenced Steps

Each step ships independently and has its own rollback story.

1. **Staff schema parity.** Add `address_bidx`, `address_digest`, `number_bidx`, `number_digest`
   columns and partial-unique indexes (`WHERE *_bidx IS NOT NULL`) to `staff_emails` and
   `staff_telephones` (`db/operators_migrate/`). Mirror the user-side shape exactly.

2. **Backfill.** Recompute `IdentifierBlindIndex` for every existing user and staff row via a runner
   script or data migration — **not via `save`**, because doing so would prematurely rewrite the
   encrypted column under non-deterministic mode while lookups still rely on deterministic.

3. **Query path migration audit.** Find every place that queries `user_email`, `user_telephone`,
   `staff_email`, or `staff_telephone` by encrypted value (sign-in, OTP, admin search, rake tasks,
   tests). Switch each to `*_bidx` / `*_digest` lookups via the class method described above.
   **Check the audit list into the PR** so re-auditing later is not necessary.
...
4. **Switch off `deterministic: true`.** Remove the flag from all four declarations listed above.
   Hoist `set_*_digests` callbacks into the shared concerns so user / staff / customer share the
   same digest pipeline. **Sequence with the customer plan** because the concern flip affects
   customer simultaneously.

5. **Re-encrypt.** Run `bin/rails db:encryption:reencrypt` (or a zero-downtime equivalent) on both
   principal and operator databases so existing ciphertext is rewritten under the non-deterministic
   scheme. This repo now provides `db:encryption:reencrypt` backed by
   `IdentifierEncryptionReencrypt` for user / staff / customer rows.

6. **Key-rotation drill.** Add a new key, re-encrypt, drop the old key, prove sign-in / OTP /
   sign-up work end-to-end on every surface (`app`, `org`, `com`). This is the actual reason this
   migration exists; do not skip.
...
- `UserEmail#set_address_digests` (`app/models/user_email.rb:115-119`) — template callback.
- `CustomerEmail#set_address_digests` (`app/models/customer_email.rb:104-108`) — template.
