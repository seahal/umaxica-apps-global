...
**4a. Welcome route dedup (item 5).** Acme has both
`get :welcome, to: "welcomes#show", as: :welcome_entry` and `resources :welcomes, only: :show`
(`config/routes/acme.rb`, app/com/org). Welcome is a single entry page, not an id-bearing
collection. Collapse toward: `resource :welcome, only: :show, controller: "welcomes"`. **Before
editing:** the helper `acme_app_welcome_entry_url` is used by cross-surface redirect logic
(`app/controllers/concerns/authentication_redirects.rb:157-163`). Preserve a `welcome_entry`-named
route (or update all `*_welcome_entry_url` call sites + tests) so the post-auth redirect keeps
working. Trace helper usage and view links before removing either route.

**4b. Preference routing simplification (item 6).** Acme preference block enumerates ~11
single-screen `resource :x, controller: "screens", defaults: {preference_screen: "x"}` lines.
Collapse to a single parametric resource validated by a controller allowlist, e.g.:
`resources :screens, only: %i(edit update), param: :screen`, with `Preference::ScreensController`
(`AcmePreferenceScreenDispatch`) enforcing an allowlist of the existing screen keys (region,
timezone, language, currency, date, time, motion, density, page_size, theme, cookie). Keep `reset`
and `emails`. **Constraint:** preference write authorization (`authorize_preference_write!` in
`PreferencesBaseController`) and the `preference_screen` dispatch contract must be preserved; if a
named-route helper is referenced by views/tests, add explicit route aliases or update call sites.
Note: per ADR, preference authority **stays on Acme** — this is structural cleanup only.
