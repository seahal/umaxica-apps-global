...
runtime read cache. `Actor.preference` is the request-local immutable runtime preference. It is
...
- Do not issue JWTs from `Actor.preference`.
...
- Rails normal runtime reads must use `Actor.preference`.
- `Actor.preference` should be built from the access-token `prf` claim in the normal path, then
...
- If `lx=en` is present while the JWT says `lx=ja`, the request renders with
  `Actor.preference.language == "en"`, but the database and JWT stay `ja`.
...
The side-effect reflection step must read `Actor.preference` only. It should not inspect
...
access-token JWT before `Actor.preference` is initialized. It is not enabled for normal pages,
...
- Some locale/theme/timezone reflection code still reads params, cookies, JWT payload, and
  controller instance variables directly instead of resolving through `Actor.preference`.
- Current code treats request params as side-effect inputs in multiple places instead of applying
  them once as an `Actor.preference` request overlay.
...
3. Change `ActorSupport#resolved_current_preference` to prefer JWT `prf` for the normal request
   path.
4. Add a single request-overlay step that rebuilds `Actor.preference` with valid explicit `lx`,
   `ct`, and `tz` values. This step must not write DB/JWT/cookies.
5. Move locale, timezone, and theme side-effect reflection to read only from `Actor.preference`.
6. Keep the logged-in HTML preference edit entry refresh as an explicit DB -> JWT sync before
   `Actor.preference` initialization.
...
- Unit tests for the request overlay proving `lx`, `ct`, and `tz` affect only `Actor.preference` for
  the current request.
- Unit tests or controller tests proving logged-in HTML preference edit entry copies actor-local DB
  values into the current surface preference token before `Actor.preference` initialization.
...
- Controller tests proving side-effect reflection reads `Actor.preference` and not params, cookies,
