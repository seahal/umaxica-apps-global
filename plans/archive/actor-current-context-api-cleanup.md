...
Completed / retired to archive (2026-06-01).

スコープ全項目が実装済みで残作業なしのため active から退役。検証（2026-06-01, grep ベース）:

- `Actor.surface` / `Actor.domain` / `Actor.session` / `Actor.token` …
  app・lib・test とも 0 件（削除済）。
- `Current.*` / `Jumper` / `Acmeer` / `Signer` の残骸 … 0 件。
- `ActiveSupport::CurrentAttributes` を持つのは `app/models/actor.rb` のみ（facade 一本化済）。
- `context.{authn,authz,configuration,preferences,tld}` を直接読む非 Actor コード … 0 件。
- プランが唯一の残務とした "migration-only direct readers" … 該当コード・マーカーとも存在せず。
- facade 本体は `adr/actor-current-facade.md` 通り（`tld` / `authn` / `authz` / `configuration` /
  `preferences` / `step_up`）。

Controller lifecycle work that depended on this Actor API is tracked in
`plans/active/controller-boundary-lifecycle-unification.md`（こちらは別途 Deprecated 済み）。ランタイム API ターゲットとして
`Actor.authn.login_public_id` を参照する `plans/backlog/gh610-decouple-session-id-from-token.md`
は引き続き backlog で有効。
