...
> **Status (2026-05-06):** Rejected and archived. The "all session-like preferences in one `setting`
> DB" target was abandoned. Only `com_preference_*` was migrated to `setting`; `app_preference_*`
> stays in `principal` and `org_preference_*` stays in `operator`. The dual-write / dual-read layer
> (`Preference::StorageAdapter`) introduced for this plan has been removed. Per
> `adr/preference-soft-bubble-doctrine.md` (2026-05-06), the preference databases stay separate by
> design — this plan's premise no longer holds. Treat as historical only.
