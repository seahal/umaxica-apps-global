...
Decisions confirmed with the user in the original session:

- New browser/content surfaces (Base, Help, Docs, News) are each scaffolded as **app/com/org
  triples**.
- Palm is **app-only**. Native clients are only planned for the `app` audience, so `com` and `org`
  Palm endpoints are intentionally absent.
...
For each **new browser/content surface × variant** = {base, help, docs, news} × {app, com, org},
plus Palm app only. Mirror the existing `core/<variant>` minimal shape (BareController-backed
...
1. **Routes:** add `config/routes/{base,palm,help,docs,news}.rb` (one host-constrained scope per
   browser/content variant and Palm app only, each with `root`, `health` + nested, `robots`); add
   `draw :base`/`:palm`/`:help`/`:docs`/ `:news` to `config/routes.rb`. Reuse host env-var naming
   convention (`BASE_SERVICE_URL`/`BASE_CORPORATE_URL`/`BASE_STAFF_URL`, `PALM_SERVICE_URL`, and
   existing `HELP_*`/`DOCS_*`/`NEWS_*` already referenced by the OIDC registry) with
...
3. **`SIGN_ISSUERS` on `id.umaxica.*`:** confirm this is the documented WebAuthn URL-binding case,
   not Sign-as-IdP.
4. **Palm app-only:** Palm is a non-browser native API and native clients are only planned for the
   app audience. Do not add or restore Palm `com` or `org` endpoints without a new decision.
