...
**Implemented / verified 2026-05-10.** `Sign::Org::Settings::GooglesController#create` prepares the
social-auth intent directly and redirects to the OmniAuth authorize path. The remaining
...
`Sign::Org::Settings::GooglesController#create`, which calls
...
session, completes the round-trip, and returns the user to `sign_org_settings_path` with a success
flash.
...
- `app/controllers/sign/org/settings/googles_controller.rb` — rewrite `create` to inline intent
  setup (Option C from design).
- `app/controllers/sign/app/settings/googles_controller.rb` — confirm whether a `create` action
...
- `app/controllers/concerns/social_auth_concern.rb` — read-only reference. Confirm
  ...
  their signatures.
- `app/views/sign/org/settings/googles/show.html.erb` — verify the form POSTs (CSRF token present);
  fix if it currently uses a GET link.
- `test/controllers/sign/org/settings/googles_controller_test.rb` — add controller-level redirect
  tests.
...
2. **Verify the originating form does POST.** Read
   `app/views/sign/org/settings/googles/show.html.erb`. The Google button must submit a POST form
   with CSRF token (a `button_to` or `form_with` calling `sign_org_settings_google_path` with
   `method: :post`). Fix if currently a GET link.

3. **Rewrite `Sign::Org::Settings::GooglesController`:**
...
4. **Mirror in `Sign::App::Settings::GooglesController`** if a `create` exists, using provider
...
6. **Tests:**
   ...
   - Integration test (`test/integration/sign/org/google_social_login_flow_test.rb`):
     ...
     Drive the full POST `/sign/settings/google` → `/auth/google_org` → `/auth/google_org/callback`
     flow and assert the final redirect lands on `sign_org_settings_google_path` (or the configured
     success path) with a success flash. Use the staff fixture whose email matches
     `staff@example.test`.

## Verification

- `bin/rails test test/controllers/sign/org/settings/googles_controller_test.rb test/integration/sign/org/google_social_login_flow_test.rb`
  — green.
- Manual: with a real Google client id, log in as staff on `id.org.localhost`, click "Connect
  Google" — confirm the round-trip lands back at `sign_org_settings_google_path` with a success
  flash.
