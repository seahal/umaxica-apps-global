...
> `www.*`); regional surfaces (docs, news, help) live in a separate repository. The Foundation
> engine no longer exists as a target. Do not follow this plan. See
...
- tests and fixtures that still use `main.*` or `wwww.*`
...
- no Foundation localhost default uses `main.*` or `wwww.*`
