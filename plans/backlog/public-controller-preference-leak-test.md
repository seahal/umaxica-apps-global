...
The plan specified that public controller tests must verify no `Current` / `Jumper` / `Preference` /
...
missing. Additionally, `Current.preference` reset behavior is not explicitly verified for any of the
three bases.
...
- After a GET request to a public endpoint, `Current.session` is unchanged from before the request.
- After a GET request to a jump public endpoint, `Jumper.domain` is `nil`.
- `Current.preference` is not populated (remains `NULL`) after a public endpoint request — the
