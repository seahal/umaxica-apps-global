...
> POST-only (`resource :session, only: :create`), because a GET entry can be triggered by a link
> and is login CSRF (CVE-2015-9284). Base reaches it through
> `POST /social/authentication/continuation`, which issues a grant and 307s to the Auth host. The
> stale `only: :new` copies of these routes on the Base host pointed at actions that no longer
> exist and have been deleted from `config/routes/base.rb`. The rest of this section still stands.
