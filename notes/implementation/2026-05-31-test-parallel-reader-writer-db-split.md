...
with `replica: true` so Rails database tasks and parallel worker schema reconstruction include
those reader databases. They use the same migration path and schema dump as the writer so
