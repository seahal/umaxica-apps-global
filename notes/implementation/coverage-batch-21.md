...
- Starting Rails line coverage: 91.0965% (45,019 / 49,419 lines) from the repository report at
  batch start.
...
The ending SimpleCov report excluded one result older than the 600-second merge timeout, so the
line denominator changed between the two reports. The percentage is reported as emitted by the
official full Rails run.
...
- `bundle exec rubocop -a` (pre-existing unrelated corrections were reverted; remaining offenses
...
- The external-path warnings emitted during the Rails run were treated as runtime context only;
  no external files were inspected.
...
- Avoid expanding into authentication, token, OIDC, controller workflow, or destructive-flow
  targets without a separate safety review.
