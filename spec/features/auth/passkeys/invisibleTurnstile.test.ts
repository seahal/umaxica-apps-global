// The invisible-widget token solver every fetch-driven passkey ceremony solves before it calls the
// server. Waiting for the Cloudflare script is covered by `spec/lib/turnstile.test.ts`; what
// belongs here is what this module decides: where it mounts the widget, what it hands `render`, and
// that every Turnstile outcome surfaces as either the resolved token or the caller's error message.
...
describe("solveInvisibleTurnstile", () => {
  ...
  it("rejects and reports retryable when the widget reports an error", async () => {
    const promise = solveInvisibleTurnstile("site-key", ERROR_MESSAGE, null);

    await vi.waitFor(() => expect(render).toHaveBeenCalledTimes(1));
    const outcome = rendered[0]?.options["error-callback"]?.("network-error");
  ...
  it("rejects with the caller's error message when the widget times out", async () => {
    const promise = solveInvisibleTurnstile("site-key", ERROR_MESSAGE, null);
  ...
  it("rejects with the caller's error message when the browser is unsupported", async () => {
    const promise = solveInvisibleTurnstile("site-key", ERROR_MESSAGE, null);
