...
Acme is the identity and account-control plane for Project Umaxica. It owns identity, preference,
account, organization, and avatar infrastructure, but it does not own SNS product domain behavior
such as feeds, posts, follows, blocks, or mutes.
...
Preference authority may also be in conflict with older ADRs. That conflict is out of scope for this
decision and must be updated in a separate ADR.
...
5. Sign up bootstrap must create exactly one initial Account and exactly one initial natural-person
   Organization atomically, together with the membership/link records required to connect them.
6. Identity-only, Account-only, or dangling membership states are not the intended steady state for
   the bootstrap path.
...
10. Public lookup is supported through `public_id`, but Rails routes do not use `param: :public_id`.
...
13. Account and Organization should both move toward a `title` attribute as the initial short name.
