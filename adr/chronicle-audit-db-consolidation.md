...
- the former Com preference database for `settings_preference_activities`
...
- `ComPreferenceActivity` is named like an activity record but is tied to preference persistence
...
- Move `settings_preference_activities` out of preference persistence and into the chronicle domain.
...
- The old `activity` versus `behavior` distinction stops being part of the public persistence
  vocabulary.
- `ComPreferenceActivity` stops being a naming exception and becomes part of the chronicle domain.
