...
3. Primary authentication through email OTP, passkey, secret, or social login.
...
- Primary auth method (`email`, `passkey`, `secret`, `social`, etc.).
...
If a token is `ACTIVE` but the sequence is incomplete, a guard should only allow the current required
sign-in sequence paths, for example session management, checkpoint, dashboard, and logout. Other
normal routes should redirect to the current required gate or fail closed.
...
## Primary Method Security Notes

Approximate primary-authentication strength discussed:

1. Passkey.
2. Social login.
3. Secret/recovery-code login.
4. Email OTP.

Email OTP is still generally stronger than password-only email/password login because it avoids
password reuse and credential stuffing. Its weakness is that the mailbox becomes the authenticator:
mailbox compromise, forwarding rules, device notifications, shared devices, and six-digit online
guessing risk all affect security. The long-term product direction should prefer passkeys while
keeping email OTP as a practical fallback.
