...
class IdentityTotpCeremonyGrant
  TOKEN_TYPE = "totp-ceremony-grant+jwt"
  PURPOSE = "totp_ceremony"
  ...
  def self.decode(token, issuer_id:, now: Time.current)
    ...
    payload, header = IdentityTotpCeremonyContract.decode_verified_payload(
      token: token,
      issuer_id: issuer_id,
      issuer: IdentityTotpCeremonyContract.acme_issuer(surface),
      audience: IdentityTotpCeremonyContract.sign_audience(surface),
  ...
  def validate!(now: Time.current)
    IdentityTotpCeremonyContract.validate_common_payload!(
      ...
      purpose: PURPOSE,
      audience: IdentityTotpCeremonyContract.sign_audience(payload["surface"]),
      issuer: IdentityTotpCeremonyContract.acme_issuer(payload["surface"]),
      now: now,
    )
  end
  ...
  def self.default_claims(attributes, now:)
    ...
    {
      "typ" => TOKEN_TYPE,
      "iss" => IdentityTotpCeremonyContract.acme_issuer(surface),
      "aud" => IdentityTotpCeremonyContract.sign_audience(surface),
      "purpose" => PURPOSE,
      "iat" => now.to_i,
    }
  end
