...
class IdentityTelephoneCeremonyResultConsumer
  Consumption = Data.define(:transaction, :result)

  def initialize(transaction:, issuer_id: nil, now: Time.current)
    @transaction = transaction
    @issuer_id = issuer_id || IdentityTelephoneCeremonyContract.sign_issuer_id(transaction.surface)
    @now = now
  end

  def call(token)
    result = IdentityTelephoneCeremonyResult.decode(token, issuer_id: issuer_id, now: now)
  ...
  def require_match!(result, key, expected)
    ...
    raise IdentityTelephoneCeremony::Error, "#{key} does not match transaction"
  end
  ...
  def consume_result!(result)
    ...
    raise IdentityTelephoneCeremony::Error, "durable telephone ceremony transaction is required"
  end
end
