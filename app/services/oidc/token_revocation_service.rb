...
module Oidc
  class TokenRevocationService < ApplicationService
    Result =
    ...
    def revoke_access_token
      ...
      payload = Authentication::TokenService.decode_allow_expired(
        token,
        host: host.presence || Oidc::Issuer.host_for_client(client),
    ...
    def client_resource_type
      @client_resource_type ||= Oidc::Issuer.resource_type_for_client(Oidc::ClientRegistry.find!(client_id))
    end
