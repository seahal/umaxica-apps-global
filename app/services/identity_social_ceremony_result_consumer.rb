...
class IdentitySocialCeremonyResultConsumer
  ...
  def call(token)
    result = IdentitySocialCeremonyResult.decode(token, issuer_id: IdentitySocialCeremonyContract.sign_issuer_id(transaction.surface), now: now)
  ...
  def validate_result_binding!(result)
    ...
    raise IdentitySocialCeremonyContract::Error, "result surface does not match transaction" unless result["surface"].to_s == transaction.surface
    raise IdentitySocialCeremonyContract::Error, "result actor does not match transaction" unless result["actor_ref"].to_s == transaction.actor_ref
    ...
    raise IdentitySocialCeremonyContract::Error, "result provider does not match transaction" unless result["provider"].to_s == transaction.provider
    raise IdentitySocialCeremonyContract::Error, "result transaction does not match transaction" unless result["transaction_id"].to_s ==
      transaction.transaction_id
    raise IdentitySocialCeremonyContract::Error, "result grant does not match transaction" unless result["grant_jti"].to_s == transaction.grant_jti
  end
end
