...
class IdentityPasskeyCeremonyResultIssuer
  ...
  def issue!
    validate_grant!
    IdentityPasskeyCeremonyResult.issue(result_claims, issuer_id: IdentityPasskeyCeremonyContract.sign_issuer_id(surface), now: now)
  end
  ...
  def validate_grant!
    ...
    raise IdentityPasskeyCeremonyContract::Error, "grant surface does not match ceremony" unless grant["surface"].to_s == surface
    raise IdentityPasskeyCeremonyContract::Error, "grant actor does not match ceremony" unless grant["actor_ref"].to_s == actor_ref
    raise IdentityPasskeyCeremonyContract::Error, "grant session does not match ceremony" unless grant["session_ref"].to_s == session_ref
    raise IdentityPasskeyCeremonyContract::Error, "grant operation does not match ceremony" unless grant["operation"].to_s == operation
    raise IdentityPasskeyCeremonyContract::Error, "grant jti does not match transaction" unless grant["jti"].to_s == transaction.grant_jti.to_s
  ...
  def grant
    @grant ||= IdentityPasskeyCeremonyGrant.decode(grant_token, issuer_id: IdentityPasskeyCeremonyContract.acme_issuer_id(surface), now: now)
  end
