...
module Health
  DeadlineExceeded = Class.new(StandardError) unless const_defined?(:DeadlineExceeded, false)
  ...
  class Readiness
    ...
    def build_report
      ...
      Health::Report.aggregate(profile: profile, probe: :ready, checks: checks)
    rescue Health::DeadlineExceeded
      Health::Report.aggregate(
        profile: profile,
        probe: :ready,
        checks: [Health::Check::Result.new(kind: :deadline, status: :unready, message: "Deadline exceeded")],
      )
    end
