...
module Identity
  module TelephoneCeremony
    class Transaction
      ...
      def validate!
        Contract.fetch_surface_value(Contract::ACME_ISSUERS, surface)
        raise Error, "operation is invalid" unless Contract::OPERATIONS.include?(operation)
        raise Error, "actor_ref is required" if actor_ref.blank?
        raise Error, "session_ref is required" if session_ref.blank?
        raise Error, "transaction_id is required" if transaction_id.blank?
        raise Error, "grant_jti is required" if grant_jti.blank?
        raise Error, "status is invalid" unless [STATUS_PENDING, STATUS_CONSUMED].include?(status)
        raise Error, "result_jti is required for consumed transaction" if consumed? && result_jti.blank?
      end
    end
  end
end
