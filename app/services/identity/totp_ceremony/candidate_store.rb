...
module Identity
  module TotpCeremony
    class CandidateStore
      ...
      DEFAULT_TTL = Transaction::DEFAULT_TTL
      ...
      def store!(surface:, actor_ref:, session_ref:, private_key:, title:, last_otp_at:, expires_at:)
        raise Error, "TOTP candidate secret is required" if private_key.blank?
      ...
      def fetch!(ref)
        ...
        raise Error, "TOTP candidate is not found" if payload.blank?

        candidate = candidate_from(payload)
        raise Error, "TOTP candidate is expired" if candidate.expires_at.to_i <= Time.current.to_i
