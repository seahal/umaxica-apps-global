  ...
  module SocialCeremony
    class CandidateStore
      ...
      def store!(surface:, actor_ref:, session_ref:, transaction_id:, operation:, provider:, auth_hash:, expires_at:)
        raise Error, "social auth candidate is required" if auth_hash.blank?
      ...
      def fetch!(ref)
        ...
        raise Error, "social auth candidate is not found" if payload.blank?

        candidate = candidate_from(payload)
        raise Error, "social auth candidate is expired" if candidate.expires_at.to_i <= Time.current.to_i
