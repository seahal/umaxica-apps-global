...
module Identity
  module StepUpCeremony
    class FreshnessCommitter
      ...
      def validate!
        raise Error, "token is required" if token.blank?
        raise Error, "result actor does not match current actor" unless result["actor_ref"].to_s == token_actor_ref
        raise Error,
              "result session does not match current session" unless result["session_ref"].to_s == token.public_id.to_s
        raise Error, "result scope does not match requirement" unless result["scope"].to_s == expected_scope
        raise Error, "result method does not match ceremony" unless result["method"].to_s == expected_method
        raise Error, "result AAL is insufficient" unless aal_rank(result["aal"]) >= aal_rank(expected_aal)
      end
      ...
      def aal_rank(value)
        Contract::AALS.index(value.to_s) || -1
      end
      ...
      def surface
        @surface ||= Contract.decode_unverified_payload(result_token)["surface"].to_s
      end
    end
  end
end
