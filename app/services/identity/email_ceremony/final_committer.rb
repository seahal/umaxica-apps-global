  ...
  module EmailCeremony
    class FinalCommitter
      ...
      def validate_candidate_before_consumption!
        raise Error, "email candidate is already verified" unless status_value == config.fetch(:unverified_status)
        raise Error, "email candidate digest does not match result" if result["normalized_email_digest"].present? &&
      ...
      def commit_candidate!
        config.fetch(:record_class).transaction do
          ...
          raise Error, "email candidate owner changed" unless locked.public_send(config.fetch(:owner_key)) == actor.id
          raise Error, "email candidate is already verified" unless locked.public_send(config.fetch(:status_key)) ==
      ...
      def find_candidate!
        ...
        raise Error, "email candidate is required" if candidate_ref.blank?
