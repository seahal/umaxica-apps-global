...
class IdentityTotpCeremonyGrantIssuer
  ...
  def self.issue!(surface:, actor_ref:, session_ref:, operation:, credential_candidate_ref: nil,
    ...
    transaction =
      IdentityTotpCeremonyReplayStore.for(surface).create_transaction!(
        surface: surface,
        actor_ref: actor_ref,
  ...
  def initialize(transaction:, issuer_id: nil, now: Time.current)
    ...
    @issuer_id = issuer_id || IdentityTotpCeremonyContract.acme_issuer_id(transaction.surface)
    @now = now
  end

  def call
    IdentityTotpCeremonyGrant.issue(transaction.grant_claims(now: now), issuer_id: issuer_id, now: now)
  end
