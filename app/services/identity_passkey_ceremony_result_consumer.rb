...
class IdentityPasskeyCeremonyResultConsumer
  ...
  def call(token)
    result = IdentityPasskeyCeremonyResult.decode(token, issuer_id: IdentityPasskeyCeremonyContract.sign_issuer_id(transaction.surface), now: now)
  ...
  def validate_result_binding!(result)
    ...
    raise IdentityPasskeyCeremonyContract::Error, "result surface does not match transaction" unless result["surface"].to_s == transaction.surface
    raise IdentityPasskeyCeremonyContract::Error, "result actor does not match transaction" unless result["actor_ref"].to_s == transaction.actor_ref
    ...
    raise IdentityPasskeyCeremonyContract::Error, "result transaction does not match transaction" unless result["transaction_id"].to_s ==
      transaction.transaction_id
    raise IdentityPasskeyCeremonyContract::Error, "result grant does not match transaction" unless result["grant_jti"].to_s == transaction.grant_jti
  end
end
