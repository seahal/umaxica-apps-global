...
module VerificationCookieable
  ...
  # VerificationBase#verification_record_satisfied?; the matching writer was removed
  # as unused (no caller in app code). Gated on
  # JitSessionCookieConfig.force_secure? so the prefix is applied in every
  ...
  class_methods do
    def cookie_name
      JitSessionCookieConfig.force_secure? ? "#{HOST_COOKIE_PREFIX}#{COOKIE_BASENAME}" : COOKIE_BASENAME
    end
  end
end
