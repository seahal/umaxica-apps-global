...
module EmailCeremonyTransactionable
  ...
  included do
    ...
    validates :transaction_id, :surface, :actor_ref, :session_ref, :operation, :status, :grant_jti, :expires_at,
              presence: true
    validates :surface, inclusion: { in: Identity::EmailCeremony::Contract::SURFACES }
    validates :operation, inclusion: { in: Identity::EmailCeremony::Contract::OPERATIONS }
    ...
    def create_transaction!(surface: ceremony_surface, actor_ref:, session_ref:, operation:, transaction_id: nil,
      ...
      connection_owner.connected_to(role: :writing) do
        create!(
          ...
          expires_at: expires_at || (now + Identity::EmailCeremony::Transaction::DEFAULT_TTL),
          created_at: now,
          updated_at: now,
        )
      end
  ...
  def consume_result!(result_jti:, consumed_at: Time.current)
    self.class.connection_owner.connected_to(role: :writing) do
      self.class.transaction do
        ...
        raise Identity::EmailCeremony::Error, "transaction is already consumed" if locked.consumed?
        raise Identity::EmailCeremony::Error, "transaction is expired" if locked.expired?(now: consumed_at)
    ...
    raise Identity::EmailCeremony::Error, "result_jti has already been consumed: #{e.message}"
  end
