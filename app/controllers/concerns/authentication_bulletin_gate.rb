...
module AuthenticationBulletinGate
  ...
  def issue_bulletin!(kind: "checkpoint", state: "new", payload: {})
    ...
    session[AuthenticationBase::BULLETIN_SESSION_KEY] = {
  ...
  def bulletin_expired?
    ...
    Time.current.to_i >= issued_at + AuthenticationBase::BULLETIN_TIMEOUT.to_i
  end
  ...
  def refresh_bulletin_dimension!(state: "updated")
    ...
    session[AuthenticationBase::BULLETIN_SESSION_KEY] = data.merge(
      "issued_at" => Time.current.to_i,
      "state" => state.to_s,
    )
  end
  ...
  def consume_bulletin!
    ...
    session.delete(AuthenticationBase::BULLETIN_SESSION_KEY)
  end
  ...
  def bulletin_association_for_resource
    ...
    case resource
    when AuthenticationClient then resource.client_bulletins
    when AuthenticationOperator then resource.staff_bulletins
    else
