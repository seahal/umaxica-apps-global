...
module Auth
  module App
    module Sign
      module In
        class GuardsController < ::Auth::App::ApplicationController
          ...
          def guard_entry_path
            sign_app_sign_in_path(ri: current_region_identifier)
          end

          def route_guard_cycle(cycle)
            return redirect_to(sign_app_sign_in_session_path(ri: current_region_identifier, pt: guard_pt_for(cycle))) if
          ...
          def redirect_to_guard_check(cycle)
            redirect_to(sign_app_sign_in_check_path(ri: current_region_identifier, pt: guard_pt_for(cycle)))
          end
