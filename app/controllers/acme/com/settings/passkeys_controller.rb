...
module Acme
  module Com
    module Settings
      class PasskeysController < Acme::Com::ApplicationController
        ...
        def enrollment
          ...
          redirect_to(
            new_sign_com_settings_passkey_url(
              ri: params[:ri],
        ...
        def update
          ...
          if @passkey.update(update_params)
            redirect_to(
              acme_com_settings_passkey_path(@passkey, ri: params[:ri]),
        ...
        def destroy
          authorize!(@passkey)
          return redirect_last_method unless AuthMethodGuard.can_remove_passkey?(current_visitor, @passkey)
