...
module Acme
  module Com
    module Settings
      class EmailsController < Acme::Com::ApplicationController
        ...
        def update
          ...
          if @user_email.update(email_preference_params)
            redirect_to(
              edit_acme_com_settings_email_path(@user_email.public_id, ri: params[:ri]),
              notice: t("sign.com.settings.email.update.success"),
              status: :see_other,
            )
          else
            flash.now[:alert] = t("sign.com.settings.email.update.failure")
            render(:edit, status: :unprocessable_content)
          end
        end
        ...
        def destroy
          ...
          if @user_email.undeletable?
            redirect_to(
              acme_com_settings_emails_path(ri: params[:ri]),
