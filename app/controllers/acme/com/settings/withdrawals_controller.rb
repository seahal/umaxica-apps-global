...
module Acme
  module Com
    module Settings
      class WithdrawalsController < Acme::Com::ApplicationController
        ...
        def withdrawal_new_path(extra_params = {})
          new_acme_com_settings_withdrawal_path({ ri: params[:ri] }.merge(extra_params))
        end

        def withdrawal_edit_path
          edit_acme_com_settings_withdrawal_path(ri: params[:ri])
        end

        def withdrawal_settings_path
          acme_com_settings_withdrawal_path(ri: params[:ri])
        end
