    ...
    class SessionLimitResolutionsController < Acme::App::ApplicationController
      ...
      def update
        ...
        unless token_belongs_to_actor?(token)
          @form_error = t("acme.app.session_limit_resolutions.revoke_failed")
          load_session_inventory
        ...
        if hard_reject_still_applies?
          @form_notice = t("acme.app.session_limit_resolutions.capacity_still_full")
          load_session_inventory
          return render :show, status: :unprocessable_content
        end
      ...
      def render_invalid_resolution
        render plain: t("acme.app.session_limit_resolutions.invalid_or_expired"), status: :gone
      end
      ...
      def load_social_resolution
        ...
        payload = Rails.application.message_verifier(:social_session_limit_resolution).verify(@social_resolution_token)
      ...
      def promote_social_resolution_session
        if current_session&.restricted?
          ...
          login_result = log_in(
            ...
            audit_context: { auth_method: "social_session_limit_resolution" },
