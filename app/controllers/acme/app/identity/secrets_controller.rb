    ...
    module Identity
      class SecretsController < BaseController
        ...
        def index
          ...
          render "sign/app/settings/secret_credentials/index"
        end

        def show
          set_secret_credential
          authorize!(@secret_credential)
          render "sign/app/settings/secret_credentials/show"
        end
        ...
        def new
          ...
          render "sign/app/settings/secret_credentials/new"
        end

        def edit
          set_secret_credential
          authorize!(@secret_credential)
          render "sign/app/settings/secret_credentials/edit"
        end
        ...
        def create
          ...
          redirect_to(
            acme_app_identity_secret_path(result.secret_credential.public_id, ri: params[:ri]),
            status: :see_other,
          )
        end
        ...
        def update
          ...
          result.secret_credential.errors.empty? ? redirect_to(
            acme_app_identity_secret_path(
              result.secret_credential.public_id,
              ri: params[:ri],
            ), status: :see_other,
          ) : render("sign/app/settings/secret_credentials/edit", status: :unprocessable_content)
        end
        ...
        def destroy
          ...
          authorize!(secret_credential)
          return redirect_to(
            acme_app_identity_secrets_path(ri: params[:ri]),
            status: :see_other,
          ...
          redirect_to(acme_app_identity_secrets_path(ri: params[:ri]), status: :see_other)
        end
