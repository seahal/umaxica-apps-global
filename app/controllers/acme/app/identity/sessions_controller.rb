    ...
    module Identity
      class SessionsController < BaseController
        ...
        def index
          ...
          render "sign/app/settings/sessions/index"
        end
        ...
        def destroy
          return redirect_to(
            acme_app_identity_sessions_path(ri: params[:ri]),
            status: :see_other,
          ) if current_session_record?(@session)

          revoke_selected_session!(@session)
          redirect_to(acme_app_identity_sessions_path(ri: params[:ri]), status: :see_other)
        end
