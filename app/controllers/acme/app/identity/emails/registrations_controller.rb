    ...
    module Identity
      module Emails
        class RegistrationsController < BaseController
          ...
          def after_email_registration_started_path(params = {}) = edit_acme_app_identity_emails_registration_path(params)

          def new_email_registration_path(params = {}) = new_acme_app_identity_emails_registration_path(params)

          def after_email_registration_verified_path
            email_registration_return_path(
              acme_app_identity_emails_url(
                ri: params[:ri], host: ENV.fetch("ACME_SERVICE_URL", "www.app.localhost"),
              ),
            )
          end
