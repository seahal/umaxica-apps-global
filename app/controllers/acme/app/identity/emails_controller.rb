    ...
    module Identity
      class EmailsController < BaseController
        ...
        def edit
          ...
          render "sign/app/settings/emails/edit"
        end
        ...
        def update
          ...
          unless cloudflare_turnstile_stealth_validation["success"]
            ...
            return render("sign/app/settings/emails/edit", status: :unprocessable_content)
          end
          if @user_email.update(email_preference_params)
            redirect_to(edit_acme_app_identity_email_path(@user_email.public_id, ri: params[:ri]), status: :see_other)
          else
            @user_email.errors.add(:base, t("sign.app.settings.email.update.failure"))
            render("sign/app/settings/emails/edit", status: :unprocessable_content)
          end
        end
        ...
        def destroy
          ...
          if @user_email.undeletable? || (verified_email?(@user_email) && !AuthMethodGuard.can_remove_email?(
            ...
            return redirect_to(acme_app_identity_emails_path(ri: params[:ri]), status: :see_other)
          end

          @user_email.destroy!
          create_audit_event!(ClientChronicleEvent::EMAIL_REMOVED, subject: @user_email)
          redirect_to(acme_app_identity_emails_path(ri: params[:ri]), status: :see_other)
        end
