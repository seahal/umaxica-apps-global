...
module Acme
  module App
    module Sign
      module In
        # Acme sign-in limitation ceremony for OIDC resume and social handoff.
        class LimitationsController < Acme::App::ApplicationController
          ...
          def update
            ...
            unless token_belongs_to_actor?(token)
              @form_error = t("acme.app.sign.in.limitations.revoke_failed")
              load_session_inventory
            ...
            if hard_reject_still_applies?
              @form_notice = t("acme.app.sign.in.limitations.capacity_still_full")
              load_session_inventory
              return render :show, status: :unprocessable_content
            end
          ...
          def destroy
            ...
            if social_resolution?
              return redirect_to(
                sign_app_sign_in_url(host: ENV.fetch("SIGN_SERVICE_URL", "id.app.localhost")),
          ...
          def render_invalid_resolution
            render plain: t("acme.app.sign.in.limitations.invalid_or_expired"), status: :gone
          end
