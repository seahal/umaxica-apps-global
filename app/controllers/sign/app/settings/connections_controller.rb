...
module Sign
  module App
    module Settings
      class ConnectionsController < Sign::App::ApplicationController
        ...
        def connection_path_for(connection)
          sign_app_settings_connection_path(connection.public_id, ri: params[:ri])
        end

        def connections_path
          sign_app_settings_connections_path(ri: params[:ri])
        end
