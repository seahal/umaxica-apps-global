    ...
    module In
      class GuardsController < Sign::App::ApplicationController
        ...
        def route_guard_cycle(cycle)
          return redirect_to(sign_app_in_session_path(ri: params[:ri], pt: guard_pt_for(cycle))) if
        ...
        def redirect_to_guard_check(cycle)
          redirect_to(sign_app_in_check_path(ri: params[:ri], pt: guard_pt_for(cycle)))
        end
