  ...
  module Org
    class ApplicationController < ActionController::Base
      ...
      rate_limit(
        ...
        scope: "sign_org_default_web",
        name: "default_web",
        store: rate_limit_store,
        with: -> { render_rate_limited(rule_name: "sign_org_default_web", retry_after: 60) },
      )
      ...
      # Direct Sign entrypoints keep signed-in operators on the credential host.
      def after_login_path
        return oidc_authorization_after_login_path if oidc_authorization_login_challenge.present?

        sign_org_dashboard_path(ri: current_region_identifier)
      end
