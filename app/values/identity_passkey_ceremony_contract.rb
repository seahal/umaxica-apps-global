...
module IdentityPasskeyCeremonyContract
  ...
  SIGN_AUDIENCES = {
    "app" => "https://log.umaxica.app/passkey-ceremony",
    "com" => "https://log.umaxica.com/passkey-ceremony",
    "org" => "https://log.umaxica.org/passkey-ceremony",
  }.freeze

  ACME_AUDIENCES = {
    "app" => "https://www.umaxica.app/passkey-ceremony-result",
    "com" => "https://www.umaxica.com/passkey-ceremony-result",
    "org" => "https://www.umaxica.org/passkey-ceremony-result",
  }.freeze
  ...
  def fetch_surface_value(values, surface)
    ...
    raise IdentityPasskeyCeremonyContract::Error, "surface is invalid"
  end
  ...
  def validate_keys!(payload, allowed:)
    ...
    raise IdentityPasskeyCeremonyContract::Error, "forbidden claims: #{forbidden.sort.join(", ")}" if forbidden.present?

    unknown = keys - allowed
    raise IdentityPasskeyCeremonyContract::Error, "unknown claims: #{unknown.sort.join(", ")}" if unknown.present?
  end

  def validate_required!(payload, required)
    missing = required.reject { |key| payload[key].present? }
    raise IdentityPasskeyCeremonyContract::Error, "missing required claims: #{missing.join(", ")}" if missing.present?
  end

  def validate_exact!(payload, key, expected)
    raise IdentityPasskeyCeremonyContract::Error, "#{key} is invalid" unless payload[key].to_s == expected.to_s
  end

  def validate_inclusion!(payload, key, allowed)
    raise IdentityPasskeyCeremonyContract::Error, "#{key} is invalid" unless allowed.include?(payload[key].to_s)
  end

  def validate_binding!(payload)
    raise IdentityPasskeyCeremonyContract::Error, "actor_ref is required" if payload["actor_ref"].blank?
    raise IdentityPasskeyCeremonyContract::Error, "session_ref is required" if payload["session_ref"].blank?
  end

  def validate_timestamp!(payload, key)
    Integer(payload[key])
  rescue ArgumentError, TypeError
    raise IdentityPasskeyCeremonyContract::Error, "#{key} must be an integer timestamp"
  end

  def validate_future_timestamp!(payload, key, now:)
    value = Integer(payload[key])
    raise IdentityPasskeyCeremonyContract::Error, "#{key} is expired" unless value > now.to_i
  rescue ArgumentError, TypeError
    raise IdentityPasskeyCeremonyContract::Error, "#{key} must be an integer timestamp"
  end
  ...
  def validate_header!(header, expected_type:)
    raise IdentityPasskeyCeremonyContract::Error, "header is invalid" if header.blank?
    raise IdentityPasskeyCeremonyContract::Error, "alg is invalid" unless header["alg"] == ALGORITHM
    raise IdentityPasskeyCeremonyContract::Error, "typ is invalid" unless header["typ"] == expected_type
    raise IdentityPasskeyCeremonyContract::Error, "kid is required" if header["kid"].blank?
    raise IdentityPasskeyCeremonyContract::Error, "unsafe header is forbidden" if %w(crit jku jwk x5u).any? { |key| header.key?(key) }
  end
  ...
  def decode_unverified_payload(token)
    ...
    raise IdentityPasskeyCeremonyContract::Error, "token is invalid: #{e.message}"
  end
  ...
  def decode_verified_payload(token:, issuer_id:, issuer:, audience:, expected_type:, required:)
    ...
    raise IdentityPasskeyCeremonyContract::Error, "kid is unknown" if public_key.blank?
    ...
    raise IdentityPasskeyCeremonyContract::Error, "token verification failed: #{e.message}"
  end
end
