...
This document records Rails entrypoints that do not require an already signed-in actor.
Every entrypoint is private by default unless its concrete controller/action declares
...
The category IDs below are enforced by
`test/unit/security/public_entrypoint_inventory_test.rb`. Adding a new `:bare`, `:open`, or
`:guest` route requires either fitting one of these categories or updating this document and the
test together.
