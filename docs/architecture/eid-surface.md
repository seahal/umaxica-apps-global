...
`eid.umaxica.net` is the dedicated UMAXICA Entity Identifier service surface. An EID is a
...
EID does not mean UUID, ULID, GUID, DOI, URI, or database primary key. One of those mechanisms may
later support an implementation, but none is part of the EID domain contract today. This bootstrap
also defines no DOI-, Handle-, or ARK-compatible protocol.

## Identifier Invariants

- One EID identifies one entity or resource.
- An EID must never be reused for a different entity or resource.
...
The underlying resource owns its domain data and lifecycle. EID is the stable reference concept and
...
authoritative EID model or store exists, every transport-safe EID currently returns the standard
...
See [the EID ADR](../../adr/eid-entity-identifier-surface.md) for the decision and compatibility
consequences.
