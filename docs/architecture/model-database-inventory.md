...
This document is a current-state inventory of model and database placement in Project Umaxica.
...
does so later placement decisions can be made against current facts instead of memory or assumptions.
...
| -------------- | ----------------------------- | --------------------- | -------------- | ---------- | ----------- | ----- |
| `app_principal` | `config/database.yml` / `POSTGRESQL_APP_PRINCIPAL_PUB`, `POSTGRESQL_APP_PRINCIPAL_SUB` | `development_app_principal_db`, `test_app_principal_db` | `db/app_principals_migrate` | Principal-side identity, credential, lifecycle, transitional state, and retained storage | retained storage / regional-ready application data | Current rows include `Client`, `Member`, credential models, and related transitional tables. |
| `app_principal_replica` | `config/database.yml` / `POSTGRESQL_APP_PRINCIPAL_SUB` | replica of `app_principal` | `db/app_principals_migrate` | read replica | read replica | Replica only. |
| `app_zenith` | `config/database.yml` / `POSTGRESQL_APP_ZENITH_PUB`, `POSTGRESQL_APP_ZENITH_SUB` | `development_app_zenith_db`, `test_app_zenith_db` | `db/app_zenith_migrate` | Zenith authority / projection | target authority | Current app-side account, identity binding, content, and bridge rows live here. |
| `app_zenith_replica` | `config/database.yml` / `POSTGRESQL_APP_ZENITH_SUB` | replica of `app_zenith` | `db/app_zenith_migrate` | read replica | read replica | Replica only. |
...
| `com_principal` | `config/database.yml` / `POSTGRESQL_COM_PRINCIPAL_PUB`, `POSTGRESQL_COM_PRINCIPAL_SUB` | `development_com_principal_db`, `test_com_principal_db` | `db/com_principals_migrate` | Principal-side identity, credential, lifecycle, transitional state, and retained storage | retained storage / regional-ready application data | Current visitor-side authority rows live here. |
| `com_principal_replica` | `config/database.yml` / `POSTGRESQL_COM_PRINCIPAL_SUB` | replica of `com_principal` | `db/com_principals_migrate` | read replica | read replica | Replica only. |
| `com_zenith` | `config/database.yml` / `POSTGRESQL_COM_ZENITH_PUB`, `POSTGRESQL_COM_ZENITH_SUB` | `development_com_zenith_db`, `test_com_zenith_db` | `db/com_zenith_migrate` | Zenith authority / projection | target authority | Current com-side account and identity binding rows live here. |
| `com_zenith_replica` | `config/database.yml` / `POSTGRESQL_COM_ZENITH_SUB` | replica of `com_zenith` | `db/com_zenith_migrate` | read replica | read replica | Replica only. |
...
| `org_principal` | `config/database.yml` / `POSTGRESQL_ORG_PRINCIPAL_PUB`, `POSTGRESQL_ORG_PRINCIPAL_SUB` | `development_org_principal_db`, `test_org_principal_db` | `db/org_principals_migrate` | Principal-side identity, credential, lifecycle, transitional state, and retained storage | retained storage / regional-ready application data | Current operator-side authority rows live here. |
| `org_principal_replica` | `config/database.yml` / `POSTGRESQL_ORG_PRINCIPAL_SUB` | replica of `org_principal` | `db/org_principals_migrate` | read replica | read replica | Replica only. |
| `org_zenith` | `config/database.yml` / `POSTGRESQL_ORG_ZENITH_PUB`, `POSTGRESQL_ORG_ZENITH_SUB` | `development_org_zenith_db`, `test_org_zenith_db` | `db/org_zenith_migrate` | Zenith authority / projection | target authority | Current org-side account and identity binding rows live here. |
| `org_zenith_replica` | `config/database.yml` / `POSTGRESQL_ORG_ZENITH_SUB` | replica of `org_zenith` | `db/org_zenith_migrate` | read replica | read replica | Replica only. |
...
| `search` | `config/database.yml` / `POSTGRESQL_SEARCH_PUB`, `POSTGRESQL_SEARCH_SUB` | `development_search_db`, `test_search_db` | `db/searches_migrate` | infrastructure / search | out of scope | Configured in `database.yml`, but `db/searches_migrate` does not exist in the repository. |
| `search_replica` | `config/database.yml` / `POSTGRESQL_SEARCH_SUB` | replica of `search` | `db/searches_migrate` | read replica | read replica | Path is configured but missing from the tree. |
| `storage` | `config/database.yml` / `POSTGRESQL_STORAGE_PUB`, `POSTGRESQL_STORAGE_SUB` | `development_storage_db`, `test_storage_db` | `db/storages_migrate` | infrastructure | out of scope | Configured in `database.yml`, but `db/storages_migrate` does not exist in the repository. |
| `storage_replica` | `config/database.yml` / `POSTGRESQL_STORAGE_SUB` | replica of `storage` | `db/storages_migrate` | read replica | read replica | Path is configured but missing from the tree. |

## Migration Directories

| migration path | inferred database key | migration count | newest migration | oldest migration | role | notes |
| -------------- | --------------------- | --------------: | ---------------- | ---------------- | ---- | ----- |
| `db/app_principals_migrate` | `app_principal` | 302 | `20260513153000_repair_user_passkey_status_reference_rows` | `20240827130201_create_users` | principal | Main app-side principal migration history. |
...
| `db/caches_migrate` | `cache` | 4 | `20260312100000_create_solid_cache_entries` | `20260508140999_consolidate_retention_on_catch_all` | infrastructure | Cache store. |
...
| `db/com_principals_migrate` | `com_principal` | 54 | `20240627130203_create_extensions` | `20251224171000_insert_dummy_guest_data` | principal | Main com-side principal migration history. |
...
| `db/org_principals_migrate` | `org_principal` | 220 | `20260623100000_update_operator_preference_defaults` | `20240827130202_create_staffs` | principal | Main org-side principal migration history. |
...
| `db/org_zenith_migrate` | `org_zenith` | 22 | `20260627000001_create_agent_assignments` | `20260511223457_create_staff_personnel_statuses` | zenith | Org-side authority / projection store. |
...
| `AppPrincipalRecord` | `app/models/app_principal_record.rb` | yes | `ApplicationRecord` | `connects_to database: { writing: :app_principal, reading: :app_principal_replica }` | `app_principal` | principal | App-side principal base. |
...
| `ComPrincipalRecord` | `app/models/com_principal_record.rb` | yes | `ApplicationRecord` | `connects_to database: { writing: :com_principal, reading: :com_principal_replica }` | `com_principal` | principal | Com-side principal base. |
...
| `OrgPrincipalRecord` | `app/models/org_principal_record.rb` | yes | `ApplicationRecord` | `connects_to database: { writing: :org_principal, reading: :org_principal_replica }` | `org_principal` | principal | Org-side principal base. |
...
### Principal-side current storage
...
| ----- | ---- | ----- | ------------------------ | ----------------------- | -------------- | -------------- | ---------------- | --------------- | -------- | ----- |
| `Client` | `app/models/client.rb` | `clients` | `AppPrincipalRecord` | `app_principal` | `db/app_principals_migrate` | authority | target authority | transitional | schema comment + `AppPrincipalRecord` inheritance | Runtime actor; retains credentials/session/contact/lifecycle state today. |
| `Member` | `app/models/member.rb` | `members` | `AppPrincipalRecord` | `app_principal` | `db/app_principals_migrate` | transitional | future candidate | transitional | schema comment + `AppPrincipalRecord`; includes `Account` | Main ambiguity cluster. See `docs/architecture/principal-zenith-membership-organization-placement.md` and `adr/member-client-membership-organization-decomposition-before-placement.md`. |
| `ClientMembership` | `app/models/client_membership.rb` | `client_memberships` | `AppPrincipalRecord` | `app_principal` | `db/app_principals_migrate` | transitional | future candidate | transitional | schema comment + `AppPrincipalRecord` | Transitional membership join; see `docs/architecture/principal-zenith-membership-organization-placement.md` and `adr/member-client-membership-organization-decomposition-before-placement.md`. |
| `ClientAppleIdentity` | `app/models/client_apple_identity.rb` | `client_apple_identities` | `AppPrincipalRecord` | `app_principal` | `db/app_principals_migrate` | credential | out of scope | excluded | schema comment + `AppPrincipalRecord` | Credential-side row. |
| `ClientGoogleIdentity` | `app/models/client_google_identity.rb` | `client_google_identities` | `AppPrincipalRecord` | `app_principal` | `db/app_principals_migrate` | credential | out of scope | excluded | schema comment + `AppPrincipalRecord` | Credential-side row. |
| `ClientEmail`, `ClientTelephone`, `ClientPasskey`, `ClientSecretCredential`, `ClientTotpCredential` and related status tables | various | various | `AppPrincipalRecord` | `app_principal` | `db/app_principals_migrate` | credential | out of scope | excluded | schema comments + `AppPrincipalRecord` | Credential/contact inventory remains on principal side for this placement pass. |
...
| ---- | ----------------- | ------------- | --------------- | --------------- | ---- |
| `Member` | `app_principal` | Transitional account-like row still living in principal storage. | zenith or retired bridge layer | Whether it should remain as a legacy bridge or be replaced by a settled account model. | High |
| `ClientMembership` | `app_principal` | Membership join still in principal storage, but its semantic boundary is not settled. | zenith or transitional | Whether this is authority, membership history, or a compatibility bridge. | High |
| `Organization` | `org_principal` | Current org principal hierarchy container, but target authority is zenith. | zenith | Whether the current `Organization` model is authoritative or transitional. | High |
...
| ---- | ----------------- | ---------------- | ------ |
| Principal authority | `app_principal` / `org_principal` / `com_principal` | `zenith` for authority data, with ticket/session/token out of scope | transitional |
...
| Organization authority | `app_principal` / `org_principal` / `com_principal` today, with hierarchy/projection already in zenith | `zenith` | ambiguous |
...
Principal is not empty. `app_principal`, `org_principal`, and `com_principal` still carry runtime actors,
credentials, lifecycle columns, and transitional rows.

`zenith` already contains the account and identity-binding graph for all three surfaces, plus read-only
content rows and RP bridge rows. That graph is the clearest target authority store for Account and
Identity placement.

`principal` is not required to become empty. Its retained role is compatibility storage and future
regional-ready application data, not canonical Principal authority ownership.

Avatar is still a separate actor-authority boundary. `Avatar`, `AvatarAssignment`, `AvatarMembership`,
`AvatarOwnershipPeriod`, and `AvatarPersonaBinding` remain in `avatar`, and the social graph
(`AvatarFollow`, `AvatarBlock`, `AvatarMute`) also stays there.
...
recovery rows plus OIDC connection rows. `Avatar` organization-related fields are also ambiguous
and should not be overread as settled authority signals. The retained principal role does not decide
...
`principal`. Authority ownership must follow the semantic role of the data, not the retained
storage label.
