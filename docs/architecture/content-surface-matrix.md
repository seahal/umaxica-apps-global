...
Rails owns content authority and the read API. Public HTML for these surfaces belongs to Edge, not
Rails. Current Edge runtime for the twelve content packages is TanStack Start on Cloudflare Workers.
`adr/015-public-content-surfaces-astro.md` in the Edge repository accepts an Astro migration that is
in progress and not deployed. Historical Rails ADRs still say Next.js; that is not current
