...
The current auth account area uses `/configuration` and `Sign::*::Configuration::*`. For user-facing
...
- `/configuration` account URLs on the sign app, com, and org surfaces
- `Sign::*::ConfigurationsController`
- `Sign::*::Configuration::*` account-setting controllers
- route helpers such as `sign_app_configuration_path`
- views under `app/views/sign/*/configurations` and `app/views/sign/*/configuration`
...
- `ConfigurationsController` -> `SettingsController`
...
- `/configuration/totps` and `TotpsController` remain unchanged for now. Future candidate:
...
- Redirect target tests that assert `/configuration` must be updated in the same slice.
...
The auth account area currently uses `/configuration` and `Sign::*::Configuration::*` for
user-facing account settings. In English, `configuration` reads like administrator or system setup,
while `settings` is the natural term for account preferences, security controls, sessions, passkeys,
emails, and linked identities.

Proposed direction:

- Rename `/configuration` to `/settings` across sign app, com, and org surfaces.
...
- Do not rename `/configuration/totps` to `/settings/totp-credentials` until the TOTP credential
