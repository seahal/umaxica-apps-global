# Alphabetical Ubiquitous Language

This file is the alphabetical index for DDD ubiquitous language.

Add terms under the first letter of the canonical English term. If a Japanese term is the canonical
domain term, place it under the first romanized letter and include the Japanese spelling in the term
heading.

## A

- **Account** — service-usage subject bound to an Identity. See
  `identity-account-organization-avatar.md`.
- **Avatar** — SNS-facing embodiment operated by Accounts (app/org only). See
  `identity-account-organization-avatar.md`.

## B

_No entries yet._

## C

- **Client Theme Key** — `ct`, current client theme preference transport key. See
  `preference-request-context-keys.md`.
- **Currency Key** — `cu`, display currency preference transport key. See
  `preference-request-context-keys.md`.

## D

- **Date Format Key** — `df`, date format preference transport key. See
  `preference-request-context-keys.md`.
- **Density Key** — `dn`, UI density preference transport key. See
  `preference-request-context-keys.md`.

## E

_No entries yet._

## F

_No entries yet._

## G

_No entries yet._

## H

- **Handle** — Avatar's mutable `@`-prefixed identifier (stored without `@`). See
  `identity-account-organization-avatar.md`.

## I

- **Identity** — authentication subject / runtime actor (`Client`/`Operator`/`Visitor`). See
  `identity-account-organization-avatar.md`.

## J

_No entries yet._

## K

_No entries yet._

## L

- **Language Key** — `language`, UI language preference cookie key. See
  `preference-request-context-keys.md`.

## M

- **Motion Key** — `mo`, motion preference transport key. See `preference-request-context-keys.md`.

## N

_No entries yet._

## O

- **Organization** — hierarchical organizational and billing unit (the Collective:
  `Enterprise`/`Bureau`/`Company`). See `identity-account-organization-avatar.md`.

## P

- **Page Size Key** — `ps`, page size or items-per-page preference transport key. See
  `preference-request-context-keys.md`.
- **public_id** — immutable external identifier (NanoID-21, DB-unique); internal `id` is never
  exposed. See `identity-account-organization-avatar.md`.

## Q

_No entries yet._

## R

_No entries yet._

## S

_No entries yet._

## T

- **Time Format Key** — `tf`, time format preference transport key. See
  `preference-request-context-keys.md`.
- **Timezone Key** — `tz`, timezone preference transport key. See
  `preference-request-context-keys.md`.

## U

_No entries yet._

## V

_No entries yet._

## W

_No entries yet._

## X

_No entries yet._

## Y

_No entries yet._

## Z

_No entries yet._
