...
| Infrastructure | Compose services (Postgres, Valkey, fakecloud, Loki, Tempo, Grafana), pnpm/Tailwind toolchain |
...
| Storage              | fakecloud S3 (AWS-compatible)                                                                             | Always-on local `fakecloud` Compose service with `object_storage:prepare`/`object_storage:smoke` rake tasks (`lib/tasks/object_storage.rake`) for manual verification. Shrine (`config/initializers/shrine.rb`) resolves storage per environment through `ObjectStorage::ShrineConfiguration`: `Memory` in test, S3-compatible storage in development when `OBJECT_STORAGE_*` is configured and otherwise `FileSystem` under `tmp/uploads`, and AWS S3 only in production. No attachment is declared on any model yet, so no bucket is required. Active Storage (`config/storage.yml`) remains `Disk`-only and unused. |
...
- `compose.yaml` launches the normal infrastructure, including the always-on `fakecloud` AWS
  emulator with four persistent volumes. Other volumes store data per service.
