...
| `valkey-cache`, `valkey-rate-limit`    | none                       | Reached as `valkey-cache:6379` / `valkey-rate-limit:6379`.                                                           |
...
Expected: `primary`, `replica`, both Valkey services, and `kafka` show a bare container port with no
`->` mapping. `core` shows `127.0.0.1:3000->3000/tcp` and `127.0.0.1:3036->3036/tcp`. No line
anywhere contains `0.0.0.0:3000`, `0.0.0.0:3036`, `0.0.0.0:9092`, `*:3000`, `*:3036`, or
`*:9092`.
