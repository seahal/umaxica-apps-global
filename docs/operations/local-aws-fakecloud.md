...
| The developer's host (OpenTofu, `aws` CLI, `curl`) | `http://localhost:4566` — the loopback publication in `compose.yaml`.                                                          |
...
declared in `terraform/` and applied with OpenTofu, so development exercises the same resource
...
aws --endpoint-url http://localhost:4566 kafka get-bootstrap-brokers --cluster-arn "$(
  cd terraform/environments/development && tofu output -raw msk_cluster_arn)"
```
