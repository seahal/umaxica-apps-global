...
`podman`, `docker`, `tofu`, `terraform`, and `aws` are all absent from the `core` image, so **no
runtime verification of any kind was performed.** Two things in particular have never executed even
once:
...
- [ ] `podman compose --profile observability config` still lists all five observability services
...
- [ ] `tofu plan`
- [ ] `tofu apply`
- [ ] `tofu plan` again reports **no changes** — a large diff here means a resource attribute
...
- [ ] `tofu output -raw msk_cluster_arn` returns an ARN
