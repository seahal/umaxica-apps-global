...
container's `/home/global/.gitconfig` routes `https://github.com` through `gh auth git-credential` —
the `origin` remote stays on HTTPS and is not rewritten.
...
guarantees `~/.ssh/known_hosts` exists: Podman creates a missing bind source as a _directory_, and a
