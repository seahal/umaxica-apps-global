...
`service` is the top-level service namespace, such as `sign`, `acme`, or `core`.
`version` comes from `Rails.app.revision`.
`time` is UTC ISO 8601 with milliseconds and a trailing `Z`.
