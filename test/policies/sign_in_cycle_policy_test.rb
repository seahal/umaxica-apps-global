...
class SignInCyclePolicyTest < ActiveSupport::TestCase
  ...
  test "allows primary actions before actor binding" do
    ...
    policy = ClientSignInCyclePolicy.new(cycle, user: nil)
  ...
  test "primary actions require matching actor when cycle is already actor-bound" do
    ...
    assert_not_predicate ClientSignInCyclePolicy.new(cycle, user: nil), :show_primary?
    assert_not_predicate ClientSignInCyclePolicy.new(cycle, user: create_client), :verify_primary?
    assert_predicate ClientSignInCyclePolicy.new(cycle, user: @client), :show_primary?
  end

  test "allows mfa only for matching actor-bound cycle" do
    cycle = create_cycle("MFA_PENDING", principal_id: @client.id)

    assert_predicate ClientSignInCyclePolicy.new(cycle, user: @client), :show_mfa?
    assert_predicate ClientSignInCyclePolicy.new(cycle, user: @client), :verify_mfa?
    assert_not_predicate ClientSignInCyclePolicy.new(cycle, user: nil), :show_mfa?
    assert_not_predicate ClientSignInCyclePolicy.new(cycle, user: create_client), :show_mfa?
  end
  ...
  test "guardrail requires matching actor" do
    ...
    assert_not_predicate ClientSignInCyclePolicy.new(cycle, user: nil), :run_guardrail?
    assert_not_predicate ClientSignInCyclePolicy.new(cycle, user: create_client), :run_guardrail?
    assert_predicate ClientSignInCyclePolicy.new(cycle, user: @client), :run_guardrail?
  end
  ...
  test "terminal cycles deny participant actions and cannot fail again" do
    ...
    assert_not_predicate ClientSignInCyclePolicy.new(failed, user: @client), :fail?
  end
  ...
  test "surface actor class must match cycle class" do
    ...
    assert_not_predicate ClientSignInCyclePolicy.new(cycle, user: visitor), :show_mfa?
  end
