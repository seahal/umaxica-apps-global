...
module Publishing
  class EncryptedContentStorageTest < ActiveSupport::TestCase
    ...
    test "docs/app revision ciphertext is not stored as plaintext" do
      ...
      assert_not_includes raw.fetch("title"), PLAIN_TITLE
      assert_not_includes raw.fetch("summary"), PLAIN_SUMMARY
      assert_not_includes raw.fetch("body"), PLAIN_BODY.fetch("text")
      ...
      assert_not_equal raw.fetch("title"), other_raw.fetch("title")
      assert_not_equal raw.fetch("body"), other_raw.fetch("body")
    end
