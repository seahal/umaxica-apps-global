...
# == Schema Information
#
# Table name: app_preference_adult_content_gate_options
...
class AppPreferenceAdultContentGateOptionTest < ActiveSupport::TestCase
  test "name returns nothing for NOTHING id" do
    option = AppPreferenceAdultContentGateOption.new(id: AppPreferenceAdultContentGateOption::NOTHING)

    assert_equal "nothing", option.name
  end

  test "name returns approved for APPROVED id" do
    option = AppPreferenceAdultContentGateOption.new(id: AppPreferenceAdultContentGateOption::APPROVED)

    assert_equal "approved", option.name
  end

  test "name returns deny for DENY id" do
    option = AppPreferenceAdultContentGateOption.new(id: AppPreferenceAdultContentGateOption::DENY)

    assert_equal "deny", option.name
  end

  test "name returns nil for unknown id" do
    option = AppPreferenceAdultContentGateOption.new(id: 999)
