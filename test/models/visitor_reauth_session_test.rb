...
class VisitorReauthSessionTest < ActiveSupport::TestCase
  setup do
  ...
  test "is valid with nil method" do
    assert_predicate VisitorReauthSession.new(@valid_params.merge(method: nil)), :valid?
  end

  test "is valid with known methods" do
    VisitorReauthSession::METHODS.each do |method|
      assert_predicate VisitorReauthSession.new(@valid_params.merge(method: method)), :valid?, method
    end
  end

  test "is invalid with unknown method" do
    session = VisitorReauthSession.new(@valid_params.merge(method: "sms"))
  ...
  test "is valid with pending and verified statuses" do
    VisitorReauthSession::STATUSES.each do |status|
      assert_predicate VisitorReauthSession.new(@valid_params.merge(status: status)), :valid?, status
    end
  end

  test "is invalid with removed or unknown statuses" do
    %w(CANCELLED EXPIRED UNKNOWN).each do |status|
      session = VisitorReauthSession.new(@valid_params.merge(status: status))
  ...
  test "belongs to token" do
    session = VisitorReauthSession.create!(@valid_params)

    assert_equal @visitor_token, session.visitor_token
  end

  test "duplicate token row raises record not unique" do
    VisitorReauthSession.create!(@valid_params)

    assert_raises(ActiveRecord::RecordNotUnique) do
      VisitorReauthSession.create!(@valid_params.merge(method: "email_otp", status: "VERIFIED"))
    end
  end

  test "expired? reflects lapses_at boundary" do
    assert_predicate VisitorReauthSession.new(@valid_params.merge(lapses_at: Time.current)), :expired?
    assert_not VisitorReauthSession.new(@valid_params.merge(lapses_at: 1.second.from_now)).expired?
  end

  test "pending scope returns only pending sessions" do
    other_token = VisitorToken.create!(visitor: @visitor)
    pending = VisitorReauthSession.create!(@valid_params)
    VisitorReauthSession.create!(
      @valid_params.merge(visitor_token: other_token, status: "VERIFIED", return_to: "/verified"),
    )

    assert_equal [pending], VisitorReauthSession.pending.to_a
  end

  test "same actor can have independent token-bound tickets" do
    other_token = VisitorToken.create!(visitor: @visitor)
    first = VisitorReauthSession.create!(@valid_params)
    second = VisitorReauthSession.create!(@valid_params.merge(visitor_token: other_token, return_to: "/other"))

    assert_equal [first, second].sort,
                 VisitorReauthSession.where(visitor_token_id: [@visitor_token.id, other_token.id]).to_a.sort
  end
end
