...
class OperatorTokenTest < ActiveSupport::TestCase
  ...
  test "inherits from TokenRecord" do
    assert_operator OperatorToken, :<, TokenRecord
  end

  test "signed ref lookup uses token connection owner" do
    assert_equal TokenRecord, OperatorToken.send(:connection_owner)
  end
  ...
  test "session_id preserves explicit value on create" do
    staff = Operator.create!(staff_status: OperatorIdentityStatus.find(OperatorIdentityStatus::NOTHING))
    token = OperatorToken.create!(staff: staff, session_id: "custom_sid")
  ...
  test "enforces maximum concurrent sessions per staff" do
    ...
    extra_token = OperatorToken.new(staff: staff)
  ...
  test "purgeability query returns only tokens purgeable at or before now" do
    staff = Operator.create!(staff_status: OperatorIdentityStatus.find(OperatorIdentityStatus::NOTHING))
    past_token = OperatorToken.create!(staff: staff, lapses_at: 20.minutes.ago, purge_at: 10.minutes.ago)
    future_token = OperatorToken.create!(staff: staff, lapses_at: 10.minutes.ago, purge_at: 10.minutes.from_now)

    purgeable_ids = OperatorToken.where(purge_at: ..Time.current).pluck(:id)
