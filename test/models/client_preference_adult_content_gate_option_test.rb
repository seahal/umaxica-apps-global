...
class ClientPreferenceAdultContentGateOptionTest < ActiveSupport::TestCase
  test "name returns nothing for NOTHING id" do
    option = ClientPreferenceAdultContentGateOption.new(id: ClientPreferenceAdultContentGateOption::NOTHING)

    assert_equal "nothing", option.name
  end

  test "name returns approved for APPROVED id" do
    option = ClientPreferenceAdultContentGateOption.new(id: ClientPreferenceAdultContentGateOption::APPROVED)

    assert_equal "approved", option.name
  end

  test "name returns deny for DENY id" do
    option = ClientPreferenceAdultContentGateOption.new(id: ClientPreferenceAdultContentGateOption::DENY)

    assert_equal "deny", option.name
  end

  test "name returns nil for unknown id" do
    option = ClientPreferenceAdultContentGateOption.new(id: 999)
