...
class OperatorGoogleIdentityTest < ActiveSupport::TestCase
  fixtures :operators, :operator_google_identity_statuses
  ...
  test "allows only one google identity per operator" do
    ...
    duplicate = OperatorGoogleIdentity.new(
  ...
  test "requires uid token and token expiration" do
    identity = OperatorGoogleIdentity.new(staff: operators(:one), provider: "google_org")
