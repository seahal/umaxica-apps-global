...
class SignCycleTest < ActiveSupport::TestCase
  SIGN_IN_CLASSES = [
    ClientSignInCycle,
    VisitorSignInCycle,
    OperatorSignInCycle,
  ...
  test "sign-in cycles inherit from their surface cycle records" do
    assert_operator ClientSignInCycle, :<, AppTicketRecord
    assert_operator VisitorSignInCycle, :<, ComTicketRecord
    assert_operator OperatorSignInCycle, :<, OrgTicketRecord
  end

  test "sign-up cycles inherit from their surface cycle records" do
    assert_operator ClientSignUpCycle, :<, AppTicketRecord
    assert_operator VisitorSignUpCycle, :<, ComTicketRecord
    assert_operator OperatorSignUpCycle, :<, OrgTicketRecord
  end
  ...
  test "app sign-up cycles accept app entry methods" do
    ClientSignUpCycle::ENTRY_METHODS.each do |entry_method|
      cycle = build_cycle(ClientSignUpCycle, entry_method: entry_method)
  ...
  test "com sign-up cycles accept only email and telephone entry methods" do
    %w(email telephone).each do |entry_method|
      cycle = build_cycle(VisitorSignUpCycle, entry_method: entry_method)
  ...
  test "com sign-up cycles reject social provider state" do
    cycle = build_cycle(VisitorSignUpCycle, entry_method: "email", social_provider: "google")
  ...
  test "com sign-up cycles do not expose social callback state" do
    assert_not_includes VisitorSignUpCycle::STATUSES, "SOCIAL_CALLBACK_PENDING"
    assert_not_includes VisitorSignUpCycle::STEPS, "social_callback"
    assert_not_includes VisitorSignUpCycleStatus::DEFAULTS, 36
  end
  ...
  test "sign-up cycles reject unsafe return paths" do
    ["https://example.test/dashboard", "//example.test/dashboard", "dashboard"].each do |return_to|
      cycle = build_cycle(ClientSignUpCycle, return_to: return_to)
  ...
  test "sign-up cycles reject secret material in requirement state" do
    cycle = build_cycle(
      ...
      completed_requirements: {
        ...
        "passcode" => { "access_token" => "secret-value" },
      },
    )
  ...
  test "sign-up cycles require requirement state to be an object" do
    cycle = build_cycle(ClientSignUpCycle, completed_requirements: ["birthdate"])
  ...
  test "nonce comparison uses the stored digest" do
    cycle = build_cycle(ClientSignInCycle, nonce: "nonce-one")
  ...
  test "transition_to allows forward sign-in transitions and rejects reverse transitions" do
    cycle = ClientSignInCycle.create!(cycle_attrs(ClientSignInCycle))

    cycle.transition_to!("MFA_PENDING", step: "mfa")

    assert_equal ClientSignInCycleStatus::MFA_PENDING, cycle.status_id
  ...
  test "sign-in cycle methods reject reverse transitions through Cycle::Base" do
    cycle = ClientSignInCycle.create!(cycle_attrs(ClientSignInCycle))
    cycle.advance_sign_in_to_mfa!
    ...
    error =
      assert_raises(Cycle::InvalidTransition) do
        cycle.transition_cycle_to!(
          ClientSignInCycleStatus::PRIMARY_PENDING,
          allowed_from: [ClientSignInCycleStatus::PRIMARY_PENDING],
        )
      end

    assert_match(/invalid transition/, error.message)
    assert_equal ClientSignInCycleStatus::MFA_PENDING, cycle.reload.status_id
  end
  ...
  test "sign-in cycle can fail from every non-terminal state" do
    non_terminal_statuses = ClientSignInCycle::STATUSES.except("COMPLETED", "FAILED")

    prosopite_pause do
      non_terminal_statuses.each do |status_name, status_id|
        cycle = ClientSignInCycle.create!(
          cycle_attrs(ClientSignInCycle).merge(status_id: status_id, step: step_for_status(status_name)),
        )
  ...
  test "terminal sign-in cycles do not transition again" do
    completed = ClientSignInCycle.create!(
      cycle_attrs(ClientSignInCycle).merge(
    ...
    assert_raises(Cycle::InvalidTransition) { completed.fail_sign_in! }

    failed = ClientSignInCycle.create!(
      cycle_attrs(ClientSignInCycle).merge(status_id: ClientSignInCycleStatus::FAILED, step: "failed"),
    )

    assert_raises(Cycle::InvalidTransition) { failed.advance_sign_in_to_guardrail! }
  end

  test "sign-in cycle methods reject expired cycles" do
    now = Time.zone.local(2026, 5, 19, 11, 0, 0)
    cycle = ClientSignInCycle.create!(
      cycle_attrs(ClientSignInCycle).merge(issued_at: now - 1.minute, expires_at: now),
    )

    travel_to now do
      assert_raises(Cycle::InvalidTransition) { cycle.advance_sign_in_to_mfa! }
    end
  ...
  test "sign-in transition_to rejects expired cycles" do
    ...
    travel_to now do
      assert_raises(Cycle::InvalidTransition) { cycle.transition_to!("MFA_PENDING") }
    end
  ...
  test "sign-up cycles cannot complete before sign-in handoff" do
    cycle = ClientSignUpCycle.create!(cycle_attrs(ClientSignUpCycle))
    ...
    assert_raises(Cycle::InvalidTransition) { cycle.complete_sign_up! }
  end
  ...
  test "expired reflects expires_at and discarded_at boundaries" do
    cycle = build_cycle(ClientSignInCycle, expires_at: 1.second.from_now)

    assert_not cycle.expired?

    expired = build_cycle(ClientSignInCycle, expires_at: Time.current)
  ...
  test "client sign-in cycle can belong to a client token" do
    ...
    cycle = ClientSignInCycle.create!(
      cycle_attrs(ClientSignInCycle).merge(principal_id: user.id, token: token),
    )
  ...
  def cycle_attrs(cycle_class, nonce: "nonce")
    ...
    attrs[:entry_method] = default_sign_up_entry_method(cycle_class) if cycle_class < SignUpCycleTicket
    attrs
  end
