...
class UserAuthorizationCodeTest < ActiveSupport::TestCase
  ...
  test "issue! generates code and sets expires_at" do
    code = UserAuthorizationCode.issue!(**@valid_params)
  ...
  test "valid scope returns only usable codes" do
    ...
    revoked = UserAuthorizationCode.issue!(**@valid_params)
    revoked.revoke!

    valid_codes = UserAuthorizationCode.valid.to_a
  ...
  test "verify_pkce" do
    ...
    code = UserAuthorizationCode.issue!(**@valid_params.merge(code_challenge: challenge))
