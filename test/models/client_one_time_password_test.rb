...
class ClientOneTimePasswordTest < ActiveSupport::TestCase
  def setup
    @user = Client.create!(public_id: "u_#{SecureRandom.hex(8)}", status_id: ClientStatus::NOTHING)
    ClientOneTimePasswordStatus.find_or_create_by!(id: ClientOneTimePasswordStatus::NOTHING)
    @status = ClientOneTimePasswordStatus.find(ClientOneTimePasswordStatus::ACTIVE)
  ...
  test "inherits from AppPrincipalRecord" do
    assert_operator ClientOneTimePassword, :<, AppPrincipalRecord
  end

  test "belongs to user" do
    association = ClientOneTimePassword.reflect_on_association(:user)
  ...
  test "belongs to user_one_time_password_status" do
    association = ClientOneTimePassword.reflect_on_association(:user_one_time_password_status)
  ...
  test "has private_key attribute" do
    record = ClientOneTimePassword.new(
      user: @user,
  ...
  test "enforces maximum totp records per user" do
    ...
    status = ClientOneTimePasswordStatus.find(ClientOneTimePasswordStatus::NOTHING)

    ClientOneTimePassword::MAX_TOTPS_PER_USER.times do
      ClientOneTimePassword.create!(
        user: new_user,
        user_one_time_password_status: status,
        private_key: ROTP::Base32.random_base32,
        last_otp_at: Time.current,
      )
    end

    extra_totp = ClientOneTimePassword.new(
      user: new_user,
      user_one_time_password_status: status,
      private_key: ROTP::Base32.random_base32,
      last_otp_at: Time.current,
    ...
    assert_includes extra_totp.errors[:base], "exceeds maximum totps per user (#{ClientOneTimePassword::MAX_TOTPS_PER_USER})"
  end

  test "association deletion: destroys when user is destroyed" do
    record = ClientOneTimePassword.create!(user: @user)
    @user.destroy
