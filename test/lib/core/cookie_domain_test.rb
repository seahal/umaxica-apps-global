...
class CookieDomainTest < ActiveSupport::TestCase
  ...
  test "normalize_configured returns nil for blank value" do
    ...
    assert_nil CoreCookieDomain.send(:normalize_configured, nil)
  end
  ...
  test "normalize_configured adds dot prefix" do
    result = CoreCookieDomain.send(:normalize_configured, "example.com")
  ...
  test "normalize_host lowercases and removes trailing dot" do
    assert_equal "example.com", CoreCookieDomain.send(:normalize_host, "EXAMPLE.COM")
    assert_equal "example.com", CoreCookieDomain.send(:normalize_host, "example.com.")
  end

  test "normalize_host removes port" do
    assert_equal "example.com", CoreCookieDomain.send(:normalize_host, "example.com:8080")
  end

  test "normalize_host extracts host from a URL string" do
    assert_equal "app.example.com", CoreCookieDomain.send(:normalize_host, "https://APP.EXAMPLE.COM:3000/path")
  end

  test "localhost_host? returns true for localhost" do
    assert CoreCookieDomain.send(:localhost_host?, "localhost")
  end

  test "localhost_host? returns true for subdomain of localhost" do
    assert CoreCookieDomain.send(:localhost_host?, "app.localhost")
  end

  test "localhost_host? returns false for other hosts" do
    assert_not CoreCookieDomain.send(:localhost_host?, "example.com")
  end

  test "best_effort_apex extracts apex domain" do
    assert_equal "example.com", CoreCookieDomain.send(:best_effort_apex, "app.example.com")
    assert_equal "example.com", CoreCookieDomain.send(:best_effort_apex, "wwww.example.com")
  end

  test "best_effort_apex returns nil for single part" do
    assert_nil CoreCookieDomain.send(:best_effort_apex, "localhost")
  end
end
