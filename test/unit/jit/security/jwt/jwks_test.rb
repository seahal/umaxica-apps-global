    ...
    module Jwt
      class JwksTest < ActiveSupport::TestCase
        ...
        test "parses jwk set objects" do
          parsed = Jwks.parse_public_collection(JSON.generate(keys: [@public_jwk]))

          assert_equal @public_jwk, parsed.fetch("kid-1")
        end

        test "parses jwk arrays" do
          parsed = Jwks.parse_public_collection(JSON.generate([@public_jwk]))
        ...
        test "rejects jwk set objects without keys array" do
          error =
            assert_raises(Jwks::Error) do
              Jwks.parse_public_collection(JSON.generate(keys: {}))
            end
        ...
        test "rejects private material in collection" do
          error =
            assert_raises(Jwks::Error) do
              Jwks.parse_public_collection(JSON.generate(keys: [@public_jwk.merge("d" => "secret")]))
            end
