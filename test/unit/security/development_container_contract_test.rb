...
class DevelopmentContainerContractTest < ActiveSupport::TestCase
  ...
  def remote_access_sidecar
    YAML.safe_load_file(REPOSITORY_ROOT.join("compose.custom.yaml"))
      .fetch("services")
      .fetch("tailscale-core")
  end
  ...
  test "the Tailscale sidecar needs no privilege, capability, or host access" do
    ...
    %w(privileged cap_add cap_drop devices network_mode pid userns_mode).each do |key|
  ...
  test "the Tailscale sidecar is opt-in and cannot start from a plain compose up" do
    assert_includes remote_access_sidecar.fetch("profiles"), "remote",
                    "without a profile a standard `podman compose up` would create, start, " \
                    "and pull an inbound network listener nobody asked for"
  end
  ...
  test "the Tailscale sidecar caps what a crash loop can consume" do
    ...
    %w(cpus mem_limit pids_limit logging).each do |key|
      assert sidecar.key?(key),
             "tailscale-core declares no #{key}, so a crash loop is bounded only by the host"
    end
  end
  ...
  test "the sidecar forwards only TCP, and only to core's unprivileged sshd" do
    serve = JSON.parse(REPOSITORY_ROOT.join("podman/tailscale/serve/serve.json").read)
  ...
  test "core's sshd accepts public keys only and never root" do
    sshd_config = REPOSITORY_ROOT.join("podman/core/sshd_config").read

    {
      ...
      "AllowTcpForwarding no" => "sshd must not become a pivot into the Podman networks",
  ...
  test "the docker and podman control-plane trees stay byte-identical" do
    %w(core/entrypoint.sh core/sshd_config tailscale/serve/serve.json).each do |relative_path|
  ...
  test "remote access stays off for developers who have not configured it" do
    core = YAML.safe_load_file(REPOSITORY_ROOT.join("compose.custom.yaml"))
      .fetch("services").fetch("core")

    assert_equal "${REMOTE_SSHD:-0}", core.fetch("environment").fetch("REMOTE_SSHD"),
                 "core is not profile-gated like the sidecar is, so the default must be off; " \
                 "the entrypoint only reads the key file and starts sshd when this is 1"
  end
  ...
  test "the tailnet node identity and sshd host key outlive their containers" do
    ...
    assert_includes volumes.keys, "tailscale-core-state",
    ...
    assert_includes volumes.keys, "umaxica-sshd-hostkey",
