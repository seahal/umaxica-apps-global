...
class AuthEndpointBurstRateLimitTest < ActionDispatch::IntegrationTest
  ...
  # has rolled over, spend the sustained allowance without tripping the burst rule.
  {
    ...
    test "#{surface} MFA passkey challenge answers 429 with the sustained retry hint once the 15-minute allowance is spent" do
      host = ENV.fetch(host_env)
      host! host
  ...
  test "app email sign-in code submission answers 429 with the sustained retry hint once the 15-minute allowance is spent" do
    host = ENV.fetch("PUBLIC_AUTH_SERVICE_URL")
    host! host
