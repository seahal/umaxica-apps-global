...
# id.umaxica.{app,com,org}/preference.
#
...
class PreferenceSecurityTest < ActionDispatch::IntegrationTest
  ...
  test "refresh_token URL parameter cannot replace the HttpOnly cookie" do
    ...
    get edit_sign_app_preference_region_url(ri: "jp")
    ...
    https!
    host! ENV.fetch("SIGN_SERVICE_URL", "id.umaxica.app")
    get edit_sign_app_preference_region_url(ri: "jp", refresh_token: legitimate_token)
  ...
  test "theme update pt redirect refuses absolute URLs and protocol-relative paths" do
    get edit_sign_app_preference_theme_url(ri: "jp")
    ...
    [
      ...
      patch sign_app_preference_theme_url(ri: "jp", pt: hostile_pt),
  ...
  test "internal pt path is preserved when safe" do
    ...
    patch sign_app_preference_theme_url(ri: "jp", pt: "/sign/app/preference?ri=jp"),
          params: { preference_theme: { option_id: "dr" } }

    assert_response :redirect
    assert_includes response.headers["Location"], "/sign/app/preference"
  end
end
