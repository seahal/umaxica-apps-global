...
class SideRouteContractTest < ActionDispatch::IntegrationTest
  ...
  def with_boot_config(side_service_host: ENV.fetch("SIDE_SERVICE_URL"),
    ...
    hosts = OpenStruct.new(
      ...
      core_service: OpenStruct.new(host: ENV.fetch("PUBLIC_CORE_SERVICE_URL", ENV.fetch("CORE_SERVICE_URL"))),
      core_corporate: OpenStruct.new(host: ENV.fetch("PUBLIC_CORE_CORPORATE_URL", ENV.fetch("CORE_CORPORATE_URL"))),
      core_staff: OpenStruct.new(host: ENV.fetch("PUBLIC_CORE_STAFF_URL", ENV.fetch("CORE_STAFF_URL"))),
