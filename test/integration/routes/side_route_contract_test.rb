...
class SideRouteContractTest < ActionDispatch::IntegrationTest
  ...
  def with_boot_config(side_service_host: ENV.fetch("SIDE_SERVICE_URL"),
    ...
    hosts = OpenStruct.new(
      ...
      auth_corporate: OpenStruct.new(host: ENV.fetch("PRIVATE_SIGN_CORPORATE_URL", "sign.com.localhost")),
      auth_staff: OpenStruct.new(host: ENV.fetch("PRIVATE_SIGN_STAFF_URL", "sign.org.localhost")),
