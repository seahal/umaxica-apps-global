...
class IdentifierBlindIndexTest < ActiveSupport::TestCase
  ...
  test "bidx_for_email returns consistent digest for normalized email" do
    Rails.app.creds.stub(:option, "email-address-secret") do
  ...
  test "bidx_for_email prefers email address hmac salt" do
    option =
      ->(key, default: nil) do
        (key == :EMAIL_ADDRESS_HMAC_SALT) ? "email-address-secret" : default
      end

    Rails.app.creds.stub(:option, option) do
      expected = OpenSSL::HMAC.hexdigest("SHA256", "email-address-secret", "email:user@example.com")
  ...
  test "bidx_for_email falls back to env salt when credentials are missing" do
    with_env("EMAIL_ADDRESS_HMAC_SALT" => "env-email-secret") do
      Rails.app.creds.stub(:option, nil) do
        expected = OpenSSL::HMAC.hexdigest("SHA256", "env-email-secret", "email:user@example.com")
  ...
  test "bidx_for_telephone returns consistent digest for normalized telephone" do
    Rails.app.creds.stub(:option, "telephone-number-secret") do
  ...
  test "bidx_for_telephone prefers telephone number hmac salt" do
    option =
      ->(key, default: nil) do
        (key == :TELEPHONE_NUMBER_HMAC_SALT) ? "telephone-number-secret" : default
      end

    Rails.app.creds.stub(:option, option) do
      expected = OpenSSL::HMAC.hexdigest("SHA256", "telephone-number-secret", "telephone:+819012345678")
  ...
  test "bidx_for_telephone falls back to env salt when credentials are missing" do
    with_env("TELEPHONE_NUMBER_HMAC_SALT" => "env-telephone-secret") do
      Rails.app.creds.stub(:option, nil) do
        expected = OpenSSL::HMAC.hexdigest("SHA256", "env-telephone-secret", "telephone:+819012345678")
