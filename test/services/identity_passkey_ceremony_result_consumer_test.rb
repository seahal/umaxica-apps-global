...
class IdentityPasskeyCeremonyResultConsumerTest < ActiveSupport::TestCase
  setup do
  ...
  test "decodes, validates the binding, and consumes the transaction" do
    ...
    IdentityPasskeyCeremonyResult.stub(:decode, ->(_token, **) { result }) do
      outcome = IdentityPasskeyCeremonyResultConsumer.new(transaction: transaction, now: @now).call("token")
  ...
  test "raises when the transaction is expired" do
    ...
    IdentityPasskeyCeremonyResult.stub(:decode, ->(_token, **) { valid_result_hash }) do
      assert_passkey_error("transaction is expired") do
        IdentityPasskeyCeremonyResultConsumer.new(transaction: transaction, now: @now).call("token")
      end
    end
  ...
  test "raises when the result surface does not match the transaction" do
    ...
    IdentityPasskeyCeremonyResult.stub(:decode, ->(_token, **) { valid_result_hash("surface" => "com") }) do
      assert_passkey_error("result surface does not match transaction") do
        IdentityPasskeyCeremonyResultConsumer.new(transaction: transaction, now: @now).call("token")
      end
    end
  end
  ...
  test "raises when the result actor does not match the transaction" do
    ...
    IdentityPasskeyCeremonyResult.stub(:decode, ->(_token, **) { valid_result_hash("actor_ref" => "other") }) do
      assert_passkey_error("result actor does not match transaction") do
        IdentityPasskeyCeremonyResultConsumer.new(transaction: transaction, now: @now).call("token")
      end
    end
  end
  ...
  test "raises when the result session does not match the transaction" do
    ...
    IdentityPasskeyCeremonyResult.stub(:decode, ->(_token, **) { valid_result_hash("session_ref" => "other") }) do
      assert_passkey_error("result session does not match transaction") do
        IdentityPasskeyCeremonyResultConsumer.new(transaction: transaction, now: @now).call("token")
      end
    end
  end
  ...
  test "raises when the result operation does not match the transaction" do
    ...
    IdentityPasskeyCeremonyResult.stub(:decode, ->(_token, **) { valid_result_hash("operation" => "deletion") }) do
      assert_passkey_error("result operation does not match transaction") do
        IdentityPasskeyCeremonyResultConsumer.new(transaction: transaction, now: @now).call("token")
      end
    end
  end
  ...
  test "raises when the result transaction id does not match the transaction" do
    ...
    IdentityPasskeyCeremonyResult.stub(:decode, ->(_token, **) { valid_result_hash("transaction_id" => "other") }) do
      assert_passkey_error("result transaction does not match transaction") do
        IdentityPasskeyCeremonyResultConsumer.new(transaction: transaction, now: @now).call("token")
      end
    end
  end
  ...
  test "raises when the result grant jti does not match the transaction" do
    ...
    IdentityPasskeyCeremonyResult.stub(:decode, ->(_token, **) { valid_result_hash("grant_jti" => "other") }) do
      assert_passkey_error("result grant does not match transaction") do
        IdentityPasskeyCeremonyResultConsumer.new(transaction: transaction, now: @now).call("token")
      end
    end
  end
