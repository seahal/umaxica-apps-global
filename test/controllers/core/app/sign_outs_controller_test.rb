...
class Core::App::SignOutsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "get sign out renders confirmation without mutation" do
    ...
    get core_app_sign_out_url(ri: "jp"), headers: {
      "X-TEST-CURRENT-USER" => user.id.to_s,
      "X-TEST-SESSION-PUBLIC-ID" => token.public_id,
    }

    assert_response :success
    assert_select "form[action=?][method=?]", core_app_sign_out_path(ri: "jp"), "post"
  ...
  test "post sign out redirects to acme oidc logout with 307" do
    ...
    assert_equal core_app_root_url(ri: "jp"), query["post_logout_redirect_uri"]
  end
end
