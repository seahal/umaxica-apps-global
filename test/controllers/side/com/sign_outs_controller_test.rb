...
class Side::Com::SignOutsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "get sign out renders confirmation without mutation" do
    ...
    get edit_side_com_sign_out_url(ri: "jp"), headers: {
  ...
  test "post sign out redirects to acme oidc logout with completion state" do
    ...
    post side_com_sign_out_url(ri: "jp"), headers: {
      "X-TEST-CURRENT-USER" => user.id.to_s,
      "X-TEST-SESSION-PUBLIC-ID" => token.public_id,
    }
    ...
    assert_equal ENV.fetch("ACME_CORPORATE_URL"), location.host
    assert_equal "/oidc/logout", location.path
    assert_predicate query["id_token_hint"], :present?
    assert_equal complete_side_com_sign_out_url(ri: "jp", protocol: "https"), query["post_logout_redirect_uri"]
  ...
  test "complete sign out consumes the state and renders completion" do
    ...
    get complete_side_com_sign_out_url(ri: "jp")
