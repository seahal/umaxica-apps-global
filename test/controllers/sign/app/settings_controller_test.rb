...
class Sign::App::SettingssControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should get show when logged in" do
    ...
    assert_select "a[href^=?]", sign_app_settings_telephones_path(ri: "jp")
    assert_select "a[href^=?]", sign_app_settings_birthdate_path(ri: "jp")
    assert_select "a[href^=?]", sign_app_settings_mfa_challenge_path(ri: "jp")
    assert_select "a[href^=?]", sign_app_mfa_reset_path(ri: "jp")
    assert_select "a[href^=?]", sign_app_settings_google_path(ri: "jp")
    assert_select "a[href^=?]", sign_app_settings_apple_path(ri: "jp")
    assert_select "a[href^=?]", sign_app_settings_sessions_path(ri: "jp")
    assert_select "a[href^=?]", new_sign_app_settings_withdrawal_path(ri: "jp")
    assert_select "a[href*=?]", edit_sign_app_sign_out_path(ri: "jp"),
                  text: /#{Regexp.escape(I18n.t("sign.app.settings.show.logout"))}/
  ...
  test "restricted session is blocked on settings with locked plain response" do
  ...
  test "active session can access settings normally" do
    token = ClientToken.create!(
      user: @user,
