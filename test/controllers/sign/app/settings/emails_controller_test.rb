...
class Sign::App::Settings::EmailsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should get index" do
    ...
    with_prosopite_paused { get sign_app_settings_emails_url(ri: "jp"), headers: request_headers }
  ...
  test "index requires step up when verified email exists" do
    ...
    with_prosopite_paused { get sign_app_settings_emails_url(ri: "jp"), headers: headers }
  ...
  test "edit requires step up when verified email exists" do
    ...
    with_prosopite_paused { get edit_sign_app_settings_email_url(email.public_id, ri: "jp"), headers: headers }
  ...
  test "index shows empty state and registration link when verified email does not exist" do
    ...
    assert_response :success
    assert_select "a[href=?]", new_sign_app_settings_emails_registration_path(ri: "jp")
  end
  ...
  test "edit renders delete form with region parameter" do
    ...
    with_prosopite_paused {
      get edit_sign_app_settings_email_url(email.public_id, ri: "jp"), headers: request_headers
    }

    assert_response :success
    assert_select(
      "form[action=?] input[name=?][value=?]",
      sign_app_settings_email_path(email.public_id, ri: "jp"),
      "_method",
      "delete",
      count: 1,
    )
  ...
  test "update changes optional email preferences only" do
    ...
    assert_redirected_to edit_sign_app_settings_email_url(email.public_id, ri: "jp")
  ...
  test "destroy removes email when not last method" do
    ...
    assert_difference("ClientEmail.count", -1) do
      assert_difference(
        ...
        with_prosopite_paused { delete sign_app_settings_email_url(email1, ri: "jp"), headers: request_headers }
      end
    end
  ...
  test "destroy removes unverified email even when it is not an auth method" do
    ...
    assert_difference("ClientEmail.count", -1) do
      with_prosopite_paused { delete sign_app_settings_email_url(email, ri: "jp"), headers: request_headers }
    end
  ...
  test "destroy blocks removing an undeletable email" do
    ...
    assert_redirected_to sign_app_settings_emails_url(ri: "jp")
    assert_equal I18n.t("sign.app.settings.email.destroy.protected"), flash[:alert]
  end
end
