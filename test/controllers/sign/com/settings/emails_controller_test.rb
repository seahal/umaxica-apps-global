...
class Sign::Com::Settings::EmailsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should get index" do
    with_prosopite_paused { get sign_com_settings_emails_url(ri: "jp"), headers: @headers }
  ...
  test "edit renders email preference toggles with current values" do
    ...
    with_prosopite_paused { get edit_sign_com_settings_email_url(email.public_id, ri: "jp"), headers: @headers }

    assert_response :success
    assert_select(
      "form[action=?][method=?] input[name=?][value=?]",
      sign_com_settings_email_path(email.public_id, ri: "jp"),
      "post",
      "_method",
      "patch",
      count: 1,
  ...
  test "destroy removes email when not last method" do
    ...
    assert_difference("VisitorEmail.count", -1) do
      with_prosopite_paused { delete sign_com_settings_email_url(email, ri: "jp"), headers: @headers }
    end
