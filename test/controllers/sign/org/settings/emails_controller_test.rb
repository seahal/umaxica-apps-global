...
class Sign::Org::Settings::EmailsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should get index" do
    with_prosopite_paused { get sign_org_settings_emails_url(ri: "jp"), headers: request_headers }
  ...
  test "edit renders email preference toggles with current values" do
    ...
    with_prosopite_paused {
      get edit_sign_org_settings_email_url(email.public_id, ri: "jp"), headers: request_headers
    }

    assert_response :success
    assert_select(
      "form[action=?][method=?] input[name=?][value=?]",
      sign_org_settings_email_path(email.public_id, ri: "jp"),
      "post",
      "_method",
      "patch",
      count: 1,
  ...
  test "update changes optional email preferences only" do
    ...
    assert_redirected_to edit_sign_org_settings_email_url(email.public_id, ri: "jp")
  ...
  test "destroy removes email when not last method" do
    ...
    assert_difference("OperatorEmail.count", -1) do
      with_prosopite_paused { delete sign_org_settings_email_url(email1, ri: "jp"), headers: request_headers }
    end
  ...
  test "destroy blocks removing an undeletable email" do
    ...
    assert_no_difference("OperatorEmail.count") do
      with_prosopite_paused { delete sign_org_settings_email_url(email, ri: "jp"), headers: request_headers }
    end

    assert_redirected_to sign_org_settings_emails_url(ri: "jp")
    assert_equal I18n.t("sign.org.settings.email.destroy.protected"), flash[:alert]
  end
end
