...
class Sign::Org::Configuration::OperatorLifecycleRequestsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index renders for authenticated operator" do
    get sign_org_configuration_operator_lifecycle_requests_url(ri: "jp"), headers: authenticated_headers

    assert_response :success
  end

  test "new renders withdrawal request for current operator" do
    get new_sign_org_configuration_operator_lifecycle_request_url(
  ...
  test "create stores lifecycle request" do
    assert_difference -> { OperatorLifecycleRequest.count }, 1 do
      post sign_org_configuration_operator_lifecycle_requests_url(ri: "jp"),
           params: {
