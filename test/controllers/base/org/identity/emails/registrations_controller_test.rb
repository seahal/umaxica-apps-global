...
class Base::Org::Identity::Emails::RegistrationsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "new renders the registration form" do
    get new_base_org_identity_emails_registration_url(ri: "jp", host: @host), headers: auth_headers
  ...
  test "create sends a code and moves the operator to the edit step" do
    ...
    assert_equal 1, @operator.operator_emails.reload.count
  end

  test "create re-renders the form when the address is malformed" do
    post base_org_identity_emails_registration_url(ri: "jp", host: @host),
         params: { staff_email: { raw_address: "not-an-address" } },
         headers: auth_headers

    assert_response :unprocessable_content
    assert_equal 0, @operator.operator_emails.reload.count
  end

  test "edit renders the code form while the registration session is live" do
    start_registration!

    get edit_base_org_identity_emails_registration_url(ri: "jp", host: @host), headers: auth_headers
  ...
  test "update re-renders the code form when the code does not match" do
    ...
    patch base_org_identity_emails_registration_url(ri: "jp", host: @host),
          params: { staff_email: { pass_code: "000000" } },
          headers: auth_headers

    assert_response :unprocessable_content
    assert_equal OperatorEmailStatus::UNVERIFIED, email.reload.staff_email_status_id
  end
  ...
  def unique_address
    "org-added-#{SecureRandom.hex(6)}@example.com"
  end
  ...
  def ensure_operator_reference_records!
    ...
    [
      OperatorEmailStatus::UNVERIFIED,
      OperatorEmailStatus::VERIFIED,
    ].each { |id| OperatorEmailStatus.find_or_create_by!(id: id) }
  end
