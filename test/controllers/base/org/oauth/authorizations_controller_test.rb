...
class Base::Org::Oauth::AuthorizationsControllerTest < ActionDispatch::IntegrationTest
  setup do
    @host = configured_host(:base_staff)
  end

  test "a request with no parameters is refused as an invalid request" do
    get base_org_oauth_authorization_url(host: @host), headers: host_headers(@host)
  ...
  test "a response_type other than code is refused" do
    get base_org_oauth_authorization_url(host: @host, **authorize_params(response_type: "token")),
  ...
  test "a missing code challenge is refused" do
    ...
    get base_org_oauth_authorization_url(host: @host, **params), headers: host_headers(@host)
  ...
  test "a code challenge method other than S256 is refused" do
    get base_org_oauth_authorization_url(host: @host, **authorize_params(code_challenge_method: "plain")),
        headers: host_headers(@host)
  ...
  test "a scope without openid is refused" do
    get base_org_oauth_authorization_url(host: @host, **authorize_params(scope: "profile")),
        headers: host_headers(@host)
  ...
  test "an unregistered client is refused without naming what was looked up" do
    get base_org_oauth_authorization_url(host: @host, **authorize_params(client_id: "not-a-registered-client")),
        headers: host_headers(@host)
  ...
  test "an unknown login challenge is refused with a fixed description" do
    get base_org_oauth_authorization_url(host: @host, login_challenge: SecureRandom.uuid),
        headers: host_headers(@host)
