...
class BasePublicControllerTest < ActionDispatch::IntegrationTest
  ...
  test "POST without CSRF token returns 422" do
    ...
    with_forgery_protection do
      # Sec-Fetch-Site decides first: a same-origin request needs no token, so the missing
      # token only rejects a request another site made.
      post("/test_csrf", params: { ri: "jp" }, headers: { "Sec-Fetch-Site" => "cross-site" })
    end
...
class Base::App::TestCsrfController
...
class Base::App::BasePublicControllerTest
...
# DAMP direct forgery helper for top-level public controller test.
class BasePublicControllerTest
  private
...
# DAMP direct CSRF helpers for top-level public controller test.
class BasePublicControllerTest
  private
