...
class Base::Com::Edge::V0::CookieControllerTest < ActionDispatch::IntegrationTest
  ...
  test "GET show returns 200 with boolean show_banner" do
    get base_com_edge_v0_cookie_path, as: :json
  ...
  test "PATCH update returns 200 with boolean show_banner and sets preference_consented cookie" do
    ...
    with_preference_jwt_keys(host: @host) do
      patch base_com_edge_v0_cookie_path,
  ...
  test "PATCH update without CSRF token returns 403" do
    with_forgery_protection do
      # Sec-Fetch-Site is what a token-less request is judged on first: a same-origin
      # request needs no token by design, so the missing token only decides the outcome
      # for a request that another site made.
      patch base_com_edge_v0_cookie_path,
            params: { consented: true },
            headers: json_headers(with_csrf: false).merge("Sec-Fetch-Site" => "cross-site"),
            as: :json
    end
...
class Base::Com::Edge::V0::CookieControllerTest
