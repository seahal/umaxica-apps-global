...
class Base::App::SignOutsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "get sign out renders confirmation without mutation" do
    ...
    get edit_base_app_sign_out_url(ri: "jp"), headers: {
  ...
  test "post sign out redirects to acme oidc logout with completion state" do
    ...
    assert_equal complete_base_app_sign_out_url(ri: "jp"), query["post_logout_redirect_uri"]
  ...
  test "complete sign out consumes the state and renders completion" do
    ...
    get complete_base_app_sign_out_url(ri: "jp", state: state)
