...
class Auth::App::Sign::Up::ParticipantsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "guard rejects ticket id without session binding" do
    ...
    get auth_app_sign_up_guard_email_url(ri: "jp", sid: ticket.public_id), headers: default_headers
  ...
  test "checkpoint show rejects ticket id without session binding" do
    ...
    get auth_app_sign_up_check_email_birthdate_url(ri: "jp", sid: ticket.public_id), headers: default_headers

    # The unbound ticket id is ignored, so the request restarts sign-up instead of loading it.
  ...
  test "checkpoint destroy is routed for sign up cancellation" do
    ...
    assert_equal "auth/app/sign/up/check/email/birthdates", route[:controller]
  ...
  test "social confirmation destroy is routed for sign up cancellation" do
    ...
    assert_equal "auth/app/sign/up/check/google/confirmations", route[:controller]
    assert_equal "destroy", route[:action]
  end
  ...
  def build_guard_controller(ticket)
    controller = Auth::App::Sign::Up::Guard::EmailsController.new
    ...
    controller.define_singleton_method(:auth_app_sign_up_check_email_birthdate_path) { |ri: nil, pt: nil|
...
class Auth::App::Sign::Up::ParticipantsControllerTest
