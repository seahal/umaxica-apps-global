    ...
    module Preference
      class EmailsControllerTest < ActionDispatch::IntegrationTest
        ...
        test "controller uses bare unsubscribe boundary" do
          assert_operator ::Acme::Org::Preference::EmailsController, :<, ::Acme::Org::BareController
          assert_not_operator ::Acme::Org::Preference::EmailsController, :<, ::Acme::Org::PreferencesBaseController
        end

        test "GET edit renders unsubscribe confirmation for a valid token" do
          get edit_acme_org_preference_email_path(@email, ri: "jp", token: @token)
        ...
        test "DELETE destroy turns operator promotional email off after confirmation" do
          ...
          delete acme_org_preference_email_path(@email), params: { token: @token, "cf-turnstile-response": "test" }

          assert_redirected_to edit_acme_org_preference_email_path(@email, token: @token)
        ...
        test "POST create turns operator promotional email off" do
          post acme_org_preference_email_path(@email), params: { token: @token }
        ...
        test "unknown public id does not unsubscribe operator email" do
          post acme_org_preference_email_path("missing-public-id"), params: { token: @token }
