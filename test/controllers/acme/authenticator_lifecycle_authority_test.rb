...
class AcmeAuthenticatorLifecycleAuthorityTest < ActionDispatch::IntegrationTest
  ...
  test "acme app social link intent rejects unsupported provider" do
    ...
    assert_no_difference -> { ClientSocialCeremonyTransaction.count } do
      post social_link_acme_app_settings_connections_url(provider: "github", ri: "jp", host: @acme_host),
           headers: app_headers(@acme_host)
    end
  ...
  test "acme app social unlink redirects to sign without removing apple identity" do
    ...
    delete social_unlink_acme_app_settings_connections_url(provider: "apple", ri: "jp", host: @acme_host),
           params: { "cf-turnstile-response": "test" },
           headers: app_headers(@acme_host)
  ...
  test "acme app social unlink is owner scoped" do
    @token.update!(last_step_up_scope: "social_unlink")
    other = Client.create!(status_id: ClientStatus::NOTHING, public_id: "aclso_#{SecureRandom.hex(4)}")
    other_identity = create_google_identity!(other, uid: "acme-other-google")
  ...
  test "acme app social unlink has no get route" do
    assert_raises(ActionController::RoutingError) do
      Rails.application.routes.recognize_path(
        "https://#{@acme_host}/settings/connections/social/google_app",
        method: :get,
      )
    end
  end
