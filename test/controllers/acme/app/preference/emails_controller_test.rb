    ...
    module Preference
      class EmailsControllerTest < ActionDispatch::IntegrationTest
        ...
        test "controller uses bare unsubscribe boundary" do
          assert_operator ::Acme::App::Preference::EmailsController, :<, ::Acme::App::BareController
          assert_not_operator ::Acme::App::Preference::EmailsController, :<, ::Acme::App::PreferencesBaseController
        end

        test "GET edit renders unsubscribe confirmation for a valid token" do
          get edit_acme_app_preference_email_path(@email, ri: "jp", token: @token)
        ...
        test "DELETE destroy turns promotional email off" do
          delete acme_app_preference_email_path(@email), params: { token: @token, "cf-turnstile-response": "test" }

          assert_redirected_to edit_acme_app_preference_email_path(@email, token: @token)
        ...
        test "POST create supports one-click unsubscribe without csrf token" do
          post acme_app_preference_email_path(@email), params: { token: @token }
        ...
        test "invalid token does not unsubscribe" do
          delete acme_app_preference_email_path(@email), params: { token: "invalid" }
