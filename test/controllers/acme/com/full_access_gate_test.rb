...
class Acme::Com::FullAccessGateTest < ActionDispatch::IntegrationTest
  setup do
  ...
  test "dashboard redirects authenticated unselected visitor to selector" do
    get acme_com_dashboard_url(host: @host, ri: "jp"), headers: as_visitor_headers(@visitor, host: @host)

    assert_redirected_to acme_com_selector_path(ri: "jp")
  end

  test "dashboard requests selection as json when context is missing" do
    get acme_com_dashboard_url(host: @host, ri: "jp"), headers: as_visitor_headers(
      @visitor,
      host: @host,
    ), as: :json
