    ...
    module Preference
      class EmailsControllerTest < ActionDispatch::IntegrationTest
        ...
        test "controller uses bare unsubscribe boundary" do
          assert_operator ::Acme::Com::Preference::EmailsController, :<, ::Acme::Com::BareController
          assert_not_operator ::Acme::Com::Preference::EmailsController, :<, ::Acme::Com::PreferencesBaseController
        end

        test "GET edit renders unsubscribe confirmation for a valid token" do
          get edit_acme_com_preference_email_path(@email, ri: "jp", token: @token)
        ...
        test "DELETE destroy turns visitor promotional email off after confirmation" do
          ...
          delete acme_com_preference_email_path(@email), params: { token: @token, "cf-turnstile-response": "test" }

          assert_redirected_to edit_acme_com_preference_email_path(@email, token: @token)
        ...
        test "POST create turns visitor promotional email off" do
          post acme_com_preference_email_path(@email), params: { token: @token }
        ...
        test "missing token does not unsubscribe visitor email" do
          post acme_com_preference_email_path(@email)
