...
class Apex::App::Dev::R18::SmokeTest < ActionDispatch::IntegrationTest
  setup do
    @host = ENV.fetch("APEX_SERVICE_URL", "app.localhost")
    host! @host
  end

  test "open R18 smoke page redirects anonymous GET to apex dev confirmation gate" do
    get apex_app___dev_r18_open_url(ri: "jp"), headers: browser_headers.merge("Host" => @host)
  ...
  test "open R18 smoke page allows anonymous GET after acknowledged cookie" do
    post apex_app___dev_r18_gate_url(ri: "jp"),
         params: { pt: apex_app___dev_r18_open_path(ri: "jp") },
  ...
  test "open R18 smoke page forbids anonymous POST instead of redirecting" do
    post apex_app___dev_r18_open_url(ri: "jp"), headers: browser_headers.merge("Host" => @host)

    assert_response :forbidden
  end

  test "private R18 smoke page redirects anonymous GET to sign in before R18 gate" do
    get apex_app___dev_r18_private_url(ri: "jp"), headers: host_headers(@host)

    assert_response :redirect
    assert_not_equal "/__dev/r18/gate", URI.parse(response.location).path
  end
